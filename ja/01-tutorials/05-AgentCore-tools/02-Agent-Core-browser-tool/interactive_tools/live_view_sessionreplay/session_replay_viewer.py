#!/usr/bin/env python3
"""
Bedrock Agentcore Browser セッション用の Session リプレイビューア

標準的な rrweb-{timestamp}-{sessionid} 形式で保存されたセッション録画を表示します。
ローカルサンプル録画と S3 ストリーミングの両方をサポートします。
"""
...
class SessionReplayHandler(BaseHTTPRequestHandler):
    """Session リプレイビューア用の HTTP リクエストハンドラ"""
    ...
    def _create_index_html(self, path):
        """ビューア HTML インターフェースを作成する"""
        import os
        print(f"\n======= デバッグ =======")
        print(f"index.html を作成中: {path}")
        print(f"パスが存在: {os.path.exists(path)}")
        print(f"親ディレクトリが存在: {os.path.exists(path.parent)}")
...
<body>
    ...
    <script>
        ...
        print(f"元のコンテンツ長: {len(html_content)}")
        print(f"'DOLLAR' を含む: {'DOLLAR' in html_content}")
        
        # Try to replace DOLLAR with $ just in case
        if 'DOLLAR' in html_content:
            html_content = html_content.replace('DOLLAR', '$')
            print(f"置換後、'DOLLAR' を含む: {'DOLLAR' in html_content}")
        
        print(f"ファイルに書き込み中: {path}")
        ...
        try:
            ...
            print(f"ファイル書き込み成功、サイズ: {file_size} バイト")
            
            # Verify content in file
            with open(path, 'r') as f:
                first_100 = f.read(100)
            print(f"ファイルの最初の 100 文字: {first_100}")
    ...
    def do_OPTIONS(self):
        """CORS プリフライト用の OPTIONS リクエストを処理する"""
        self.send_response(200)
        self.send_header('Access-Control-Allow-Origin', '*')
        self.send_header('Access-Control-Allow-Methods', 'GET, POST, OPTIONS')
        self.send_header('Access-Control-Allow-Headers', '*')
        ...
        try:
            ...
            for page in paginator.paginate(
                ...
                if 'CommonPrefixes' in page:
                    console.print(f"プレフィックス {self.prefix} に {len(page['CommonPrefixes'])} 個のディレクトリを発見")
            ...
            if not recordings:
                ...
                console.print(f"発見されたディレクトリ: {dirs}")
    ...
    def download_recording(self, recording_id):
        ...
        try:
            with Progress(
                ...
                console.print(f"プレフィックス {prefix} でファイルを検索中")
                ...
                # Download files
                console.print(f"{len(files_to_download)} 個のファイルをダウンロード中")
