#!/usr/bin/env python3
"""
録画とリプレイを含む完全な Browser サンプル

このサンプルは、完全な Bedrock AgentCore Browser ワークフローをデモンストレーションします:
1. 録画を有効にして Browser を作成
2. Browser セッションを開始
3. コントロールの取得/解放付きでライブビュー
4. 録画は自動的に S3 に保存
5. Session リプレイビューアで録画を表示

環境変数:
    AWS_REGION          - AWS リージョン（デフォルト: us-west-2）
    BEDROCK_AGENTCORE_ROLE_ARN    - Bedrock AgentCore 実行用の IAM ロール ARN（設定されていない場合はデフォルトパターンを使用）
    RECORDING_BUCKET    - 録画用の S3 バケット（デフォルト: session-record-test-{account_id}）
    RECORDING_PREFIX    - 録画用の S3 プレフィックス（デフォルト: replay-data）
    BEDROCK_AGENTCORE_STAGE       - Bedrock AgentCore ステージ（デフォルト: prod）

要件:
    - Bedrock AgentCore Browser を作成/管理する権限を持つ AWS 認証情報
    - S3 と Browser 操作の権限を持つ実行ロール
    - 適切な権限を持つ S3 バケット
"""
...
def create_browser_with_recording():
    """Control Plane API を使用して録画を有効にした Browser を作成する"""
...
def get_sigv4_headers(region: str, session_id: str) -> Dict[str, str]:
    """WebSocket 接続用の SigV4 認証ヘッダーを生成する"""
...
def view_recordings(s3_location):
    """カスタム SessionReplayViewer で録画されたセッションを直接表示する"""
    
    print("\n📼 Checking for recordings in S3...")
    print(f"場所: s3://{s3_location['bucket']}/{s3_location['prefix']}/")
    ...
    try:
        ...
        if 'Contents' not in response:
            print("S3 ロケーションにオブジェクトが見つかりません")
            return
        ...
        for obj in response['Contents']:
            ...
            if 'metadata.json' in key:
                ...
                print(f"メタデータを持つセッションを発見: {session_dir}")
        
        if not session_dirs:
            print("metadata.json を含むセッションディレクトリが見つかりません")
            return
        ...
        print(f"最新のセッションを使用: {latest_session}")
        
        # 使用する前に、まずカスタム S3 データソースクラスを定義
        class CustomS3DataSource:
            """既知の構造を持つ S3 録画用のカスタムデータソース"""
            ...
            def list_recordings(self):
                ...
                try:
                    ...
                    print(f"メタデータを取得中: {metadata_key}")
            ...
            def download_recording(self, recording_id):
                """S3 から録画をダウンロードする"""
                print(f"録画をダウンロード中: {recording_id}")
        ...
        class CustomSessionReplayHandler(SessionReplayHandler):
            ...
            def download_and_serve_recording(self, recording_id):
                """録画をダウンロードして提供する - HTML レスポンスの問題を修正"""
                try:
        ...
        # 修正されたハンドラでカスタムビューアを作成
        class CustomSessionReplayViewer(SessionReplayViewer):
            def start(self):
                """カスタムハンドラを使用してリプレイビューアサーバーを起動する"""
