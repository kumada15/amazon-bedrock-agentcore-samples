...
def setup_agent():
    """Memory とツールを使用してエージェントをセットアップする"""
    ...
    agent = Agent(
        hooks=[memory_hooks],
        tools=[calculator],
        system_prompt="あなたは親切なパーソナルアシスタントです。",
        model=model,
...
@click.group()
def cli():
    """カスタマーサポートアシスタント用 Memory テスト CLI"""
    pass


@cli.command()
def load_conversation():
    """長期 Memory をテストするために事前定義されたモック会話を読み込んで実行する"""
    conversations = [
...
def load_prompt(prompt):
    """ユーザー入力からカスタムプロンプトを読み込み、Memory と共に実行する"""
