...
def check_warranty_status(serial_number: str, customer_email: str = None) -> str:
    """
    シリアル番号を使用して製品の保証ステータスを確認します。

    Args:
        serial_number (str): 製品シリアル番号（8〜20文字の英数字）。
        customer_email (str, optional): 検証用の顧客メールアドレス。

    Returns:
        str: カバレッジ詳細と有効期限を含むフォーマット済み保証ステータス情報。
