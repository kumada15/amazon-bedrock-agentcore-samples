...
天気、検索、フライト、ホテル、場所のための純粋なツール関数。
エージェントロジックなし - MCP サーバーから直接呼び出される。
"""
...
def load_cities(filepath: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        print(f"警告: {filepath}に都市データベースが見つかりません")
    except Exception as e:
        print(f"都市データベースの読み込みエラー: {e}")
    return cities


def get_city_database() -> tuple[list[dict], list[str]]:
    """都市データベースと都市名リストを取得する。"""
...
def get_amadeus_token() -> str | None:
    """Amadeus API 用の OAuth トークンを取得する。"""
...
def serp_hotel_search(query: str, check_in_date: str, check_out_date: str) -> str:
    """
    SerpAPI の Google Hotels エンジンを使用してホテルを検索する。

    Args:
        query: ホテル検索クエリ（例: "パリの高級ホテル"、"タイムズスクエア近くのホテル"）
        check_in_date: YYYY-MM-DD 形式のチェックイン日
        check_out_date: YYYY-MM-DD 形式のチェックアウト日
...
def serp_flight_search(
    ...
    """
    SerpAPI の Google Flights エンジンを使用してフライトを検索する。

    Args:
        departure_id: 出発空港コード（例: "DCA"、"JFK"）
        arrival_id: 到着空港コード（例: "LGA"、"LAX"）
        outbound_date: YYYY-MM-DD 形式の出発日
        return_date: YYYY-MM-DD 形式の復路日（オプション、片道の場合は省略）

    Returns:
        フライト結果を含むフォーマット済み文字列
    """
    if not SERP_API_KEY:
        return "Error: SERP API key not configured."
