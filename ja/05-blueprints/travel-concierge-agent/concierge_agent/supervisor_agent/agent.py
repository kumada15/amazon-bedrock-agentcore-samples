...
logger.info(f"Itinerary テーブルを使用: {ITINERARY_TABLE_NAME}")

# 旅程用の DynamoDB セットアップ
...
def get_user_profile_data(user_id: str) -> str:
    """プロンプト用にフォーマットされたユーザープロファイルデータを取得。"""
    try:
...
def create_supervisor_agent(user_id: str, session_id: str) -> Agent:
    """AgentCore メモリセッションマネージャーを使用してスーパーバイザーエージェントを作成。"""
    ...
    logger.info("Gateway MCP クライアントをセットアップ中...")
    ...
    logger.info("エージェントを itinerary ツールとサブエージェント (cart + travel) で作成しました")
...
async def agent_stream(payload):
    """セッションマネージャーを使用したスーパーバイザーエージェントのメインエントリポイント。"""
