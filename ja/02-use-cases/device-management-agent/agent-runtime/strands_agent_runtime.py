"""
デバイス管理システム - エージェントランタイム

このモジュールは、Amazon Bedrock AgentCore Runtime と Strands Agents SDK を使用して、
デバイス管理システムのコア会話 AI エージェントを実装します。エージェントは
IoT デバイス管理操作のための自然言語処理、会話管理、およびツール実行を処理します。

主な機能:
    - Amazon Bedrock モデルを使用した自然言語理解
    - スライディングウィンドウによる会話コンテキスト管理
    - MCP（Model Context Protocol）ツール統合
    - リアルタイム WebSocket 通信
    - ロギングとトレーシングによる包括的なオブザーバビリティ

アーキテクチャ:
    - Strands Agent: Amazon Bedrock を使用したコア会話 AI
    - MCP Client: ツール実行のための Gateway との通信
    - Conversation Manager: コンテキストと履歴の管理
    - 認証: Amazon Cognito OAuth 統合
    - オブザーバビリティ: CloudWatch Logs と X-Ray トレーシング

環境変数:
    MCP_SERVER_URL: MCP ツールアクセス用の Gateway エンドポイント
    COGNITO_DOMAIN: 認証用の Amazon Cognito ドメイン
    COGNITO_CLIENT_ID: OAuth クライアント ID
    COGNITO_CLIENT_SECRET: OAuth クライアントシークレット
    AWS_REGION: サービス用の AWS リージョン

使用例:
    エージェントランタイムは通常サービスとしてデプロイされ、フロントエンド
    アプリケーションから WebSocket 接続でアクセスされます。自然言語クエリを
    処理し、MCP プロトコルを通じてデバイス管理操作を実行します。
"""
import os
import json
import logging
...
logger.info(f"MCP_SERVER_URL が設定されました: {MCP_SERVER_URL}")
...
def check_mcp_server():
    try:
        ...
        logger.info(f"MCP サーバーを確認中: {MCP_SERVER_URL}")
        
        # If no bearer token, try to get one from Cognito
        if not jwt_token:
            logger.info("Bearer トークンがないため、Cognito からの取得を試みています...")
            try:
                jwt_token = access_token.get_gateway_access_token()
                # Note: Not logging actual token for security reasons
                logger.info(f"Cognito トークンを取得: {'はい' if jwt_token else 'いいえ'}")
            except Exception as e:
                logger.error(f"Cognito トークンの取得中にエラーが発生しました: {str(e)}", exc_info=True)
        ...
        if jwt_token:
            ...
            try:
                ...
                logger.info(f"MCP サーバーのレスポンスステータス: {response.status_code}")
                
                has_tools = "tools" in response.text
                return has_tools
            except requests.exceptions.RequestException as e:
                logger.error(f"MCP サーバー確認中にリクエスト例外が発生しました: {str(e)}")
                return False
        else:
            # Try without token for local testing
            logger.info("Bearer トークンがないため、ヘルスエンドポイントを試しています")
            try:
                response = requests.get(f"{MCP_SERVER_URL}/health", timeout=5)
                response.raise_for_status()
                logger.info(f"ヘルスエンドポイントのレスポンスステータス: {response.status_code}")
                
                return response.status_code == 200
            except requests.exceptions.RequestException as e:
                logger.error(f"ヘルスエンドポイントへのリクエスト例外が発生しました: {str(e)}")
                return False
    except Exception as e:
        logger.error(f"MCP サーバーの確認中にエラーが発生しました: {str(e)}", exc_info=True)
        return False
...
def initialize_agent():
    try:
        ...
        if not jwt_token:
            logger.info("環境変数にトークンがないため、Cognito を試しています...")
            try:
        ...
        logger.info(f"Gateway エンドポイントを使用: {gateway_endpoint}")
        ...
        try:
            ...
            logger.info(f"ホスト名を IP アドレスに解決しました: {ip_address}")
        except Exception as e:
            logger.error(f"ホスト名の解決中にエラーが発生しました: {str(e)}")
        ...
        try:
            ...
            logger.info("MCP クライアントのセットアップが完了しました")
            
            # Enter the context manager
            mcp_client.__enter__()
            
            # Get the tools from the MCP server
            logger.info("MCP サーバーからツール一覧を取得中...")
            tools = mcp_client.list_tools_sync()
            logger.info(f"MCP サーバーから {len(tools)} 個のツールを読み込みました")
            ...
            if tools and len(tools) > 0:
                ...
                logger.info(f"利用可能なツール: {', '.join(tool_names)}")
            
        except Exception as e:
            logger.error(f"MCP クライアントのセットアップ中にエラーが発生しました: {str(e)}", exc_info=True)
            return None, None
        
        # Create an agent with these tools
        try:
            logger.info("ツールを持つ Strands Agent を作成中...")
            ...
            agent = Agent(
                ...
                system_prompt="""
                あなたはデバイスリモート管理用の AI アシスタントです。ユーザーのクエリをサポートしてください。
                デバイスリモート管理システムから実データを取得できるツールにアクセスできます。

                利用可能なツール:
                - list_devices: システム内のすべてのデバイスを一覧表示
                - get_device_settings: 特定のデバイスの設定を取得
                - list_wifi_networks: 特定のデバイスのすべての WiFi ネットワークを一覧表示
                - list_users: システム内のすべてのユーザーを一覧表示
                - query_user_activity: 期間内のユーザーアクティビティをクエリ
                - update_wifi_ssid: デバイス上の Wi-Fi ネットワークの SSID を更新
                - update_wifi_security: デバイス上の Wi-Fi ネットワークのセキュリティタイプを更新

                これらのツールを使用して、ユーザーがデバイスとアカウントを管理するのを支援してください。
                """
            )
            logger.info("エージェントが正常に作成されました")
            
            return agent, mcp_client
        except Exception as e:
            logger.error(f"エージェントの作成中にエラーが発生しました: {str(e)}", exc_info=True)
            return None, None
    except Exception as e:
        logger.error(f"エージェントの初期化中にエラーが発生しました: {str(e)}", exc_info=True)
        return None, None
...
if check_mcp_server():
    ...
    if agent:
        ...
        logger.warning("エージェントの初期化に失敗しました")
else:
    logger.warning("MCP サーバーが稼働していません。エージェントの初期化をスキップしました。")

# Function to format response for better display
def format_response(text):
    """プレーンテキストフォーマットを使用してレスポンスを見やすく整形します"""
...
def format_user_list(users):
    """プレーンテキストフォーマットを使用してユーザーリストを整形します"""
...
def format_activity_list(activities):
    """プレーンテキストフォーマットを使用してユーザーアクティビティリストを整形します"""
...
def format_wifi_networks(data):
    """プレーンテキストフォーマットを使用して WiFi ネットワークリストを整形します"""
...
def format_device_settings(settings):
    """プレーンテキストフォーマットを使用してデバイス設定を整形します"""
...
def format_wifi_update(result):
    """プレーンテキストフォーマットを使用して WiFi SSID 更新結果を整形します"""
...
def format_wifi_security_update(result):
    """プレーンテキストフォーマットを使用して WiFi セキュリティタイプ更新結果を整形します"""
    if not result or not isinstance(result, dict):
        return "No update result available."
...
def format_generic_list(items):
    """プレーンテキストフォーマットを使用して汎用オブジェクトリストを整形します"""
...
def format_generic_object(obj):
    """プレーンテキストフォーマットを使用して汎用オブジェクトを整形します"""
...
async def process_request(payload):
    """
    ストリーミングサポート付きで AgentCore Runtime からのリクエストを処理します
    これは AgentCore Runtime のエントリポイントです
    """
    ...
    try:
        ...
        logger.info(f"ユーザーメッセージを受信しました: {user_message}")
        
        # Check if agent is initialized
        if not agent:
            logger.info("エージェントが初期化されていません。MCP サーバーのステータスを確認中...")
            # Try to initialize the agent if MCP server is running
            if check_mcp_server():
                logger.info("MCP サーバーが稼働中です。エージェントの初期化を試みています...")
                agent, mcp_client = initialize_agent()
                if not agent:
        ...
        logger.info("Strands Agent でメッセージを処理中（ストリーミング）...")
        try:
            # Stream response using agent.stream_async
            stream = agent.stream_async(user_message)
            async for event in stream:
                logger.debug(f"ストリーミングイベント: {event}")
            ...
            logger.error(f"ストリーミングモードでエラーが発生しました: {str(e)}", exc_info=True)
            yield {"error": f"Error processing request with agent (streaming): {str(e)}"}
        
    except Exception as e:
        error_msg = f"Error processing request: {str(e)}"
        logger.error(f"エラー: {error_msg}", exc_info=True)
        yield {"error": error_msg}

if __name__ == "__main__":
    # Test MCP server connection at startup
    logger.info("起動時に MCP サーバー接続をテスト中...")
    try:
        jwt_token = access_token.get_gateway_access_token()
        ...
        response = requests.post(f"{MCP_SERVER_URL}/mcp", headers=headers, json=payload, timeout=10)
        logger.info(f"直接テストのレスポンスステータス: {response.status_code}")
    except Exception as e:
        logger.error(f"直接テストでエラーが発生しました: {str(e)}", exc_info=True)
