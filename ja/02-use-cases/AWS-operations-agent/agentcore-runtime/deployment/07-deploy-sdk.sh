...
echo "🚀 SDK エージェント (BedrockAgentCoreApp) をデプロイ中..."
...
if python3 deploy-sdk-runtime.py; then
    ...
    echo "   • bac-identity-provider-okta との OAuth 統合"
    echo "   • bac-gtw Gateway 用の MCP クライアント"
    echo "   • AgentCore Memory 統合"
    echo ""
    echo "💻 SDK エージェントは OAuth2 トークンと MCP Gateway を使用する準備ができました！"
    echo "   @entrypoint デコレータ付きの BedrockAgentCoreApp フレームワークを使用"
