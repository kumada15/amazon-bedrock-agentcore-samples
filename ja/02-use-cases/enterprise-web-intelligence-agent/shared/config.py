...
class AgentConfig:
    ...
    def validate(self) -> bool:
        ...
        if not self.recording_role_arn:
            print(f"警告: RECORDING_ROLE_ARN が設定されていません")
            print(f"AWS アカウント ID: {self.aws_account_id}")
            return False
        ...
        try:
            ...
            try:
                ...
                print(f"✅ S3 バケットが存在します: {self.s3_bucket}")
            except:
                ...
                print(f"✅ S3 バケットを作成しました: {self.s3_bucket}")
        except Exception as e:
            print(f"⚠️ S3 バケットを確認/作成できませんでした: {e}")
            print("手動で作成するか、権限を調整する必要がある場合があります")
