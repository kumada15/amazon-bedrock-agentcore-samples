...
def _parse_log_file(file_path: Path, pattern: Optional[str] = None):
    """ログファイルを解析し、パターンでフィルタリングします。"""
    logs = []

    if file_path.suffix == ".json":
        with open(file_path, "r") as f:
            data = json.load(f)
            # 配列と配列を含むオブジェクトの両方を処理
