...
def get_streamlit_url():
    try:
        ...
        print(
            "Resource-metadata.jsonファイルが見つかりません -- SageMaker Studio外で実行中"
        )
        domain_id = None
        space_name = None
        # sys.exit(1)
    except json.JSONDecodeError:
        print("エラー: resource-metadata.jsonのJSONフォーマットが無効です")
        sys.exit(1)
    except KeyError as e:
        print(f"エラー: 必須キー {e} がJSONに見つかりません")
        sys.exit(1)

    # Now you can use domain_id and space_name variables in your code
    # print(f"Domain ID: {domain_id}")
    # print(f"Space Name: {space_name}")
    print("Streamlitアプリケーションにログインしてテストするには、以下を使用してください")
...
def make_urls_clickable(text):
    """テキスト内の URL をクリック可能な HTML リンクに変換します。"""
...
def load_bedrock_agentcore_config():
    """.bedrock_agentcore.yaml ファイルから設定を読み込みます。"""
    ...
    try:
        ...
        # Validate required fields (agent_session_id is optional, will be generated per user session)
        if not agent_arn:
        ...
        return {
            "genesisSessionId": agent_session_id,  # Can be None, will generate unique session per user
...
class StreamingHttpBedrockAgentCoreClient:
    """リアルタイムレスポンス用の HttpBedrockAgentCoreClient のストリーミングバージョン。"""

    def __init__(self, region: str):
        """StreamingHttpBedrockAgentCoreClient を初期化します。"""
        self.region = region
    ...
    def invoke_endpoint_streaming(
        ...
        """エージェントエンドポイントを呼び出し、ストリーミングレスポンスチャンクを生成します。"""
                                    ...
                                    if json_start != -1:
                                        ...
                                        if json_end != -1:
                                            ...
                                            if (
                                                ...
                                                print(
                                                    f"抽出したテキスト: {formatted_response}"
                                                )  # デバッグ出力

                            except (json.JSONDecodeError, KeyError, IndexError) as e:
                                print(f"JSON パースエラー: {e}")
                                print(f"蓄積されたレスポンス: {accumulated_response}")
