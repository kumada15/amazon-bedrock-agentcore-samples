...
def read_config(file_path: str) -> Dict[str, Any]:
    """
    ファイルパスから設定を読み込みます。JSON、YAML、YML形式をサポートしています。

    Args:
        file_path (str): 設定ファイルへのパス

    Returns:
        Dict[str, Any]: 辞書形式の設定データ

    Raises:
        FileNotFoundError: ファイルが存在しない場合
        ValueError: ファイル形式がサポートされていないか無効な場合
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as file:
            if ext == ".json":
                ...
                # まず JSON、次に YAML を試してフォーマットを自動検出
                content = file.read()
                file.seek(0)
...
def save_customer_support_secret(secret_value):
    """AWS Secrets Managerにシークレットを保存します。"""
...
def get_or_create_cognito_pool(refresh_token=False):
    ...
    try:
        ...
        print("既存のCognito設定が見つかりません。新しく作成します...")
...
def cleanup_cognito_resources(pool_id):
    """
    ユーザー、アプリクライアント、ユーザープールを含むCognitoリソースを削除します
    """
    try:
        # 同じセッション設定を使用して Cognito クライアントを初期化
        boto_session = Session()
        region = boto_session.region_name
        cognito_client = boto3.client("cognito-idp", region_name=region)
        ...
        if pool_id:
            try:
                ...
                print("すべてのCognitoリソースのクリーンアップに成功しました")
                return True
        ...
        print(f"クリーンアップの初期化中にエラーが発生しました: {str(e)}")
        return False
...
def gateway_target_cleanup(gateway_id: str = None):
    ...
    print(f"✅ ゲートウェイ {gateway_id} を削除しました")
    ...
    try:
        ...
        for repo in repositories["repositories"]:
            if "bedrock-agentcore-customer_support_agent" in repo["repositoryName"]:
                ...
                print(f"  ✅ ECRリポジトリを削除しました: {repo['repositoryName']}")

    except Exception as e:
        print(f"  ⚠️  ランタイムクリーンアップ中にエラーが発生しました: {e}")
...
def local_file_cleanup():
    ...
    if deleted_files:
        print(f"\n📁 {len(deleted_files)} ファイルを削除しました")
    if missing_files:
        print(
            f"ℹ️  {len(missing_files)} ファイルは既に存在しませんでした: {', '.join(missing_files)}"
        )
