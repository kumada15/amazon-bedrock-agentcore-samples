...
echo "🚀 Lab 02: Lambda ZIP デプロイ (VPC 対応)"
echo "📂 作業ディレクトリ: $(pwd)"
echo ""

# Verify prerequisites
command -v python3 &> /dev/null || { echo "❌ Python3 が必要です"; exit 1; }
command -v aws &> /dev/null || { echo "❌ AWS CLI が必要です"; exit 1; }
...
if ! aws iam get-role --role-name "$ROLE_NAME" 2>/dev/null; then
    ...
    if ! aws iam create-role --role-name "$ROLE_NAME" \
        ...
        echo "❌ IAM ロールの作成に失敗しました"
        exit 1
    fi

    echo "→ ポリシーをアタッチ中..."
    aws iam attach-role-policy --role-name "$ROLE_NAME" --policy-arn "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole" || echo "⚠️  警告: AWSLambdaBasicExecutionRole のアタッチに失敗しました"
    aws iam attach-role-policy --role-name "$ROLE_NAME" --policy-arn "arn:aws:iam::aws:policy/AmazonBedrockFullAccess" || echo "⚠️  警告: AmazonBedrockFullAccess のアタッチに失敗しました"
    aws iam attach-role-policy --role-name "$ROLE_NAME" --policy-arn "arn:aws:iam::aws:policy/AmazonEC2ReadOnlyAccess" || echo "⚠️  警告: AmazonEC2ReadOnlyAccess のアタッチに失敗しました"
    aws iam attach-role-policy --role-name "$ROLE_NAME" --policy-arn "arn:aws:iam::aws:policy/CloudWatchReadOnlyAccess" || echo "⚠️  警告: CloudWatchReadOnlyAccess のアタッチに失敗しました"
...
if result['status'] == 'success':
    ...
    print(f"状態: {lmb['state']}")
    print()
    print("次へ: ノートブックのセクション 9 に進んでください")
    print("=" * 70)
else:
    print(f"❌ デプロイに失敗しました: {result.get('error')}")
    sys.exit(1)
EOF
