...
case "${1:-quick}" in
    ...
    quick)
        echo "QUICK 負荷テストを実行中 (5 分、2 リクエスト/分、バランス型)"
    ...
    standard)
        echo "STANDARD 負荷テストを実行中 (15 分、4 リクエスト/分、バランス型)"
        uv run python "$SCRIPT_DIR/run_load_test.py" \
            --duration 15 \
            --rate 4
        ;;
    ...
    extended)
        echo "EXTENDED 負荷テストを実行中 (30 分、5 リクエスト/分、バランス型)"
        uv run python "$SCRIPT_DIR/run_load_test.py" \
            --duration 30 \
            --rate 5
        ;;
    ...
    high-rate)
        echo "HIGH-RATE 負荷テストを実行中 (10 分、10 リクエスト/分、混合型)"
        uv run python "$SCRIPT_DIR/run_load_test.py" \
            --duration 10 \
            --rate 10 \
            --weather-only 20 \
    ...
    multi-tool)
        echo "MULTI-TOOL 集中テストを実行中 (15 分、3 リクエスト/分)"
        uv run python "$SCRIPT_DIR/run_load_test.py" \
            --duration 15 \
            --rate 3 \
            --multi-tool 60 \
    ...
    errors)
        echo "ERROR-INDUCING テストを実行中 (10 分、3 リクエスト/分、30% エラー)"
        uv run python "$SCRIPT_DIR/run_load_test.py" \
            --duration 10 \
            --rate 3 \
            --weather-only 20 \
    ...
    stress)
        echo "STRESS テストを実行中 (30 分、10 リクエスト/分、高負荷)"
        uv run python "$SCRIPT_DIR/run_load_test.py" \
            --duration 30 \
            --rate 10 \
            --weather-only 20 \
