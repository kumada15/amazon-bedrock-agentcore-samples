...
def _invoke(prompt: str, session_id: str):
    ...
    logger.info("*" * 80)
    logger.info("リクエストを送信中 %s", payload)
    logger.info("*" * 80)
    ...
    try:
        ...
        logger.warning("リクエスト失敗: %s", str(e))
        raise


def _cleanup(session_id: str):
    logger.info("*" * 80 + "\n")
    logger.info("コードインタープリターセッションをクリーンアップ中")
