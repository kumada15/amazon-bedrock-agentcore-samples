...
def cleanup_lab_04(region_name: str = "us-west-2", verbose: bool = True) -> None:
    ...
    if verbose:
        print("[DEBUG] Parameter Store 内の Lab 04 パラメータを検索中...")
        try:
    ...
    try:
        ...
        print("  ✓ Runtime 実行ロールを削除しました")
    except iam_client.exceptions.NoSuchEntityException:
        print("  ✓ Runtime 実行ロールは見つかりません (ok)")
    except Exception as e:
        print(f"  ⚠ Runtime ロール: {e}")
    ...
    print("\n" + "=" * 70)
    print("✅ Lab 04 のクリーンアップが完了しました")
    print("\nこれでセクション 1 から Lab 04 を再実行できます")
