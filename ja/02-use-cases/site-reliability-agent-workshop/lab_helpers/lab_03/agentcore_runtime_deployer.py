...
class AgentCoreRuntimeDeployer:
    """Strands Remediation Agent を AgentCore Runtime にデプロイするためのヘルパー"""
...
def store_runtime_configuration(runtime_arn: str, runtime_id: str = None, region: str = "us-west-2", prefix: str = "aiml301_sre_agentcore") -> None:
    ...
    print("\n" + "="*70)
    print("🔍 DEBUG: store_runtime_configuration() が呼び出されました")
    print("="*70)
    ...
    if runtime_id:
        ...
        try:
            ...
            print(f"❌ Runtime ID の保存に失敗: {e}")
            import traceback
            traceback.print_exc()
            raise
    else:
        print(f"\n⏭️  Runtime ID が提供されていません、スキップ...")

    print("\n" + "="*70)
    print("✅ store_runtime_configuration() 完了")
    print("="*70 + "\n")
