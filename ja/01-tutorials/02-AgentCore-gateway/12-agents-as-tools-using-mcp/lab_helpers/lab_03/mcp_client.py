...
class MCPClient:
    ...
    def initialize(self, client_name: str = "aiml301-mcp-client",
        ...
        if 'result' in response:
            ...
            raise RuntimeError(f"初期化に失敗しました: {response}")
            raise ValueError("初期化に失敗しました: レスポンスに結果がありません")

    def list_tools(self) -> List[Dict[str, Any]]:
        """
        Gateway から利用可能なすべての MCP ツールを一覧表示します。

        Returns:
            ツール名、説明、スキーマを含むツール定義のリスト
    ...
    def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Example:
            ...
            ...     "ddgs_search",
            ...     {"query": "AWS Bedrock 機能", "max_results": 3}
            ... )
            >>> print(result)
        """
        if not self.initialized:
