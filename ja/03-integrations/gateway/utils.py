...
def create_gateway_invoke_tool_role(role_name, gateway_id, current_arn):
    ...
    try:
        ...
        print(f"assume-roleポリシーをアタッチできませんでした: {e}")
        print("呼び出し元にiam:PutUserPolicyまたはiam:PutRolePolicyの権限があることを確認してください。")
    ...
    for i in range(max_retries):
        try:
            ...
            if "AccessDenied" in str(e):
                print(f"試行 {i+1}/{max_retries}: AccessDenied、3秒後に再試行...")
                time.sleep(3)
            else:
                raise
    else:
