...
class ServerState:
    """
    すべてのプロトコルインスタンス間で利用可能な共有サーバー状態。
    """
...
class Server:
    ...
    async def serve(self, sockets: Optional[List[socket.socket]] = None) -> None:
        ...
        message = "サーバープロセスを開始しました [%d]"
        color_message = "サーバープロセスを開始しました [" + click.style("%d", fg="cyan") + "]"
        ...
        message = "サーバープロセスを終了しました [%d]"
        color_message = "サーバープロセスを終了しました [" + click.style("%d", fg="cyan") + "]"
        logger.info(message, process_id, extra={"color_message": color_message})
    ...
    def _log_started_message(self, listeners: Sequence[socket.SocketType]) -> None:
        ...
        if config.fd is not None:  # pragma: py-win32
            sock = listeners[0]
            logger.info(
                "Uvicorn がソケット %s で実行中です（終了するには CTRL+C を押してください）",
                sock.getsockname(),
            )

        elif config.uds is not None:  # pragma: py-win32
            logger.info(
                "Uvicorn が UNIX ソケット %s で実行中です（終了するには CTRL+C を押してください）", config.uds
            )
            ...
            message = f"Uvicorn が {addr_format} で実行中です（終了するには CTRL+C を押してください）"
            color_message = (
                "Uvicorn が "
                + click.style(addr_format, bold=True)
                + " で実行中です（終了するには CTRL+C を押してください）"
            )
            logger.info(
                message,
                protocol_name,
    ...
    async def shutdown(self, sockets: Optional[List[socket.socket]] = None) -> None:
        ...
        try:
            ...
            logger.error(
                "%s 個の実行中タスクをキャンセルします。グレースフルシャットダウンのタイムアウトを超過しました",
    ...
    async def _wait_tasks_to_complete(self) -> None:
        ...
        if self.server_state.connections and not self.force_exit:
            msg = "接続が閉じるのを待機中です（強制終了するには CTRL+C を押してください）"
            logger.info(msg)
        ...
        if self.server_state.tasks and not self.force_exit:
            msg = "バックグラウンドタスクの完了を待機中です（強制終了するには CTRL+C を押してください）"
            logger.info(msg)
