...
def setup_cognito_user_pool():
    ...
    try:
        ...
        print(f"ディスカバリー URL: https://cognito-idp.{region}.amazonaws.com/{pool_id}/.well-known/openid-configuration")
    ...
    for pool in response["UserPools"]:
        if pool["Name"] == USER_POOL_NAME:
            ...
            if domain:
                ...
                print(f"ユーザープール {user_pool_id} のドメインが見つかりました: {domain} ({domain_url})")
            else:
                print(f"ユーザープール {user_pool_id} のドメインが見つかりません")
            return pool["Id"]
    ...
    if role_arn != "":
        ...
        try:
            ...
            if error.response['Error']['Code'] == "ResourceConflictException":
                ...
                print(f"AWS Lambda関数 {lambda_function_name} は既に存在します。同じARN {lambda_arn} を使用します")
...
def create_gateway_invoke_tool_role(role_name, gateway_id, current_arn):
    ...
    try:
        ...
        print("呼び出し元にiam:PutUserPolicyまたはiam:PutRolePolicy権限があることを確認してください")
    ...
    for i in range(max_retries):
        try:
            ...
            if "AccessDenied" in str(e):
                print(f"試行 {i+1}/{max_retries}: AccessDenied、3秒後にリトライします...")
                time.sleep(3)
            else:
                raise
    else:
        raise RuntimeError(f"{max_retries}回のリトライ後にロール {role_name} の引き受けに失敗しました")

    print(f"ロール '{role_name}' の準備が完了し、{current_arn} がBedrock Agent Gatewayを呼び出せるようになりました")
