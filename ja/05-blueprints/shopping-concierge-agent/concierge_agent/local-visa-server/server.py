...
def onboard_card_endpoint():
    ...
    try:
        ...
        print(f"📦 プロビジョン結果のキー: {provision_result.keys()}") # codeql[py/clear-text-logging-sensitive-data] Debug logging for certificate verification - logs metadata only, not private key content
        # codeql[py/clear-text-logging-sensitive-data] Debug logging for certificate verification - logs metadata only, not private key content
