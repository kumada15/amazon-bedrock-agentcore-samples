...
if [[ -f "web-ui/.env.local" && "$FORCE" == "false" ]]; then
    echo "⚠️  web-ui/.env.local は既に存在します"
    echo "   再生成するには --force を付けて実行してください"
    exit 0
fi

# Get CDK outputs from CloudFormation
echo "📊 ${STACK_NAME} を照会しています..."
CDK_OUTPUTS=$(aws cloudformation describe-stacks --stack-name "$STACK_NAME" --region us-east-1 --query "Stacks[0].Outputs" --output json 2>/dev/null) || {
    echo "❌ ${STACK_NAME} を照会できませんでした。先にエージェントスタックをデプロイしてください。"
    exit 1
}
...
VISA_OUTPUTS=$(aws cloudformation describe-stacks --stack-name "$VISA_STACK_NAME" --region us-east-1 --query "Stacks[0].Outputs" --output json 2>/dev/null) || {
    echo "⚠️  ${VISA_STACK_NAME} を照会できませんでした。Visa Lambda がデプロイされていません - モックモードを使用します。"
    VISA_PROXY_URL=""
}
...
echo "📊 Secrets Manager から Visa 認証情報を取得しています..."
...
if [[ ! -f "amplify_outputs.json" ]]; then
    echo "❌ amplify_outputs.json が見つかりません。先に Amplify をデプロイしてください。"
    exit 1
fi
...
if [[ -z "$RUNTIME_ARN" || -z "$USER_POOL_ID" || -z "$USER_POOL_CLIENT_ID" ]]; then
    echo "❌ 必須の値が不足しています。両方のスタックがデプロイされていることを確認してください。"
    exit 1
fi
...
echo "✅ web-ui/.env.local を作成しました"

if [[ -n "$VISA_PROXY_URL" ]]; then
    echo "✅ Visa プロキシ URL を設定しました: $VISA_PROXY_URL"
    echo "   VITE_VISA_MOCK_MODE=false"
else
    echo "⚠️  Visa Lambda がデプロイされていません - モックモードを使用します"
fi

if [[ -n "$VISA_API_KEY" ]]; then
    echo "✅ Secrets Manager から Visa API キーを読み込みました"
else
    echo "⚠️  Secrets Manager に Visa API キーが見つかりません (visa/api-key)"
fi

echo ""
echo "次のステップ: cd web-ui && npm run dev"
