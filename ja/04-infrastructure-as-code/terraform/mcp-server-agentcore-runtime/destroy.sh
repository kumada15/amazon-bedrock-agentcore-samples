...
print_warning "  - AgentCore Runtime（MCP サーバー付き）"
print_warning "  - Cognito ユーザープール（テストユーザーを含む）"
print_warning "  - S3 バケット（ソースコードストレージ）"
print_warning "  - ECR リポジトリ（すべてのイメージを含む）"
print_warning "  - CodeBuild プロジェクト"
print_warning "  - IAM ロールとポリシー"
...
if [ "$RUNTIME_COUNT" -eq 0 ]; then
    print_success "AgentCore Runtime がクリーンアップされました"
else
...
if [ "$S3_BUCKETS" -eq 0 ]; then
    print_success "S3 バケットがクリーンアップされました"
else
...
if [ "$COGNITO_POOLS" -eq 0 ]; then
    print_success "Cognito ユーザープールがクリーンアップされました"
else
    print_warning "'$STACK_NAME' に一致する Cognito ユーザープールが $COGNITO_POOLS 個見つかりました"
    print_info "手動でクリーンアップが必要な場合があります"
fi
