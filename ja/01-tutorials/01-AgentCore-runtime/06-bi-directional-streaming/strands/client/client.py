...
class StrandsClientHandler(BaseHTTPRequestHandler):
    """Strands クライアントを提供する HTTP リクエストハンドラー"""
...
""",
    ...
    # 提供されている場合はランタイム ARN からリージョンを抽出
    if args.runtime_arn:
    ...
    try:
        ...
        if args.runtime_arn:
            ...
            print(f"🎙️ 音声ID: {default_voice_id}（デフォルト）")
            print(f"🆔 セッションID: {session_id}")
            print(
                f"⏰ URLの有効期限: {args.expires}秒（{args.expires / 60:.1f}分）"
            )
            print()
            print("🔐 署名付きURLを生成中...")
