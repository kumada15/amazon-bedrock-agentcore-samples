#!/usr/bin/python
"""Strands エージェント用の AgentCore Memory 統合。"""
...
def create_or_get_memory_resource():
    try:
        ...
        try:
            ...
            print("AgentCore Memory リソースを作成しています。数分かかる場合があります...")
        ...
        if (
            ...
            try:
                ...
                if all_context:
                    ...
                    logger.info(f"{len(all_context)} 件の顧客コンテキストアイテムを取得しました")

            except Exception as e:
                logger.error(f"顧客コンテキストの取得に失敗しました: {e}")

    def save_support_interaction(self, event: AfterInvocationEvent):
        """エージェントレスポンス後にカスタマーサポートインタラクションを保存します"""
        try:
            messages = event.agent.messages
            if len(messages) >= 2 and messages[-1]["role"] == "assistant":
                # 最後の顧客クエリとエージェントレスポンスを取得
                customer_query = None
                agent_response = None
                ...
                if customer_query and agent_response:
                    ...
                    logger.info("サポートインタラクションをメモリに保存しました")

        except Exception as e:
            logger.error(f"サポートインタラクションの保存に失敗しました: {e}")

    def register_hooks(self, registry: HookRegistry) -> None:
        """カスタマーサポートメモリフックを登録します"""
        registry.add_callback(MessageAddedEvent, self.retrieve_customer_context)
        registry.add_callback(AfterInvocationEvent, self.save_support_interaction)
        logger.info("カスタマーサポートメモリフックを登録しました")
