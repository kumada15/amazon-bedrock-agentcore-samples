...
def setup_cognito_user_pool(region, memory_id):
    ...
    try:
        ...
        print(
            f"ディスカバリURL: https://cognito-idp.{region}.amazonaws.com/{pool_id}/.well-known/openid-configuration"
        )
        print(f"クライアントID: {client_id}")
        print(f"IdentityプールID: {identity_pool_info['identity_pool_id']}")
        print(f"ユーザー1のBearerトークン: {bearer_token1}")
        print(f"ユーザー2のBearerトークン: {bearer_token2}")
        print(f"ユーザー1のIDトークン: {id_token1}")
        print(f"ユーザー2のIDトークン: {id_token2}")
...
def reauthenticate_users(client_id, region, users=None):
    ...
    for username, password in users.items():
        try:
            ...
            print(f"{username}の認証に成功しました")
        except Exception as e:
            print(f"{username}の認証エラー: {str(e)}")
...
def create_agentcore_role(agent_name, region):
    ...
    print(f"ロールポリシー {agentcore_role_name} をアタッチ中")
    try:
