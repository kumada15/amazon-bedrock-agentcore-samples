...
def cleanup_lab_03(region_name: str = "us-west-2", verbose: bool = True) -> None:
    ...
    if verbose:
        ...
        try:
            ...
            if response.get('Parameters'):
                print(f"  {len(response['Parameters'])} 個のパラメータが見つかりました:")
    ...
    print("[1/7] OAuth2 Credential Provider を削除中...")
    ...
    try:
        ...
        try:
            ...
            if provider_arn:
                ...
                if verbose:
                    print(f"  ℹ Provider ARN が見つかりました: {provider_arn}")
                    print(f"  ℹ 抽出された Provider 名: {provider_name}")
                ...
                try:
                    ...
                    print(f"  ✓ OAuth2 Credential Provider を削除しました: {provider_name}")
                    ...
                    if "ResourceNotFoundException" in error_str or "does not exist" in error_str.lower():
                        print(f"  ✓ Provider は既に削除済みか見つかりません（OK）")
                        provider_deleted = True
                    else:
                        print(f"  ⚠ Provider {provider_name} の削除に失敗しました: {error_str}")

        except ssm_client.exceptions.ParameterNotFound:
            if verbose:
                print("  ℹ Provider ARN が Parameter Store に見つかりません（OK）")
            provider_deleted = True
        ...
        if oauth2_secrets:
            for secret in oauth2_secrets:
                ...
                try:
                    ...
                    print(f"  ✓ シークレットを削除しました: {secret_name}")
                except Exception as e:
                    error_str = str(e)
                    if "ResourceNotFoundException" not in error_str:
                        ...
                        if "bedrock-agentcore-identity" in error_str:
                            print(f"  ℹ シークレット {secret_name} はサービス所有です - Provider 削除時に自動削除されます")
                        else:
                            print(f"  ⚠ シークレット {secret_name} の削除に失敗しました: {error_str}")
        else:
            print("  ✓ OAuth2 m2m 認証情報シークレットが見つかりません")
        ...
        for gw in gateways.get('items', []):
            if 'remediation-gateway' in gw['name']:
                ...
                try:
                    ...
                    if target_count > 0:
                        print(f"  {target_count} 個のターゲットを削除中...")
                            ...
                            if remaining_count == 0:
                                ...
                                if retry_count < max_retries:
                                    print(
                                        ...
                                        f"{remaining_count} 個のターゲットがまだ残っています..."
                                    )
                                else:
                                    print(
                                        f"  ⚠ {max_retries} 回のリトライ後も "
                                        f"{remaining_count} 個のターゲットが関連付けられています"
                                    )
                    else:
                        print("  ✓ ターゲットが見つかりません")
                        targets_deleted = True
            ...
            try:
                ...
                for param_name in param_names:
                    try:
                        ...
                        if verbose:
                            print(f"  パラメータが見つかりました: {param_name}")
                        ...
                        if runtime_id:
                            ...
                            try:
                                ...
                                print(f"  ✓ Runtime 削除を開始しました: ****")

                                # Wait for Runtime to be fully deleted
                                print("  ⏳ Runtime 削除完了を待機中...")
                                max_retries = 60
                                retry_count = 0
        ...
        if not runtime_deleted:
            ...
            try:
                ...
                if verbose and all_items:
                    print(f"  API 経由で {len(all_items)} 個の Runtime が見つかりました")
                ...
                for rt in all_items:
                    ...
                    if 'remediation' in runtime_name or 'aiml301' in runtime_name:
                        ...
                        runtime_id_for_logging = runtime_id
                        print(f"  Runtime が見つかりました: {rt['agentRuntimeName']}")
    ...
    try:
        ...
        try:
            ...
            print(f"  SSM から Interpreter ID が見つかりました: {interpreter_id}")
        except ssm_client.exceptions.ParameterNotFound:
            if verbose:
        ...
        if not interpreter_id:
            ...
            for item in list_response.get('codeInterpreterSummaries', []):
                if 'aiml301' in item.get('name', '').lower() and 'custom' in item.get('name', '').lower():
                    ...
                    print(f"  API 経由で Interpreter が見つかりました: {interpreter_id}")
                    break
        ...
        if interpreter_id:
            try:
                ...
                if "ResourceNotFoundException" in str(e) or "not found" in str(e).lower():
                    print(f"  ✓ Code Interpreter は既に削除されています（OK）")
                else:
                    print(f"  ⚠ Code Interpreter の削除に失敗しました: {e}")
        else:
            print("  ✓ Code Interpreter が見つかりません（OK）")
    except Exception as e:
    ...
    try:
        ...
        print("  ✓ Custom Runtime 実行ロールが見つかりません（OK）")
    except Exception as e:
    ...
    try:
        ...
        print("  ✓ Code Interpreter 実行ロールが見つかりません（OK）")
    except Exception as e:
    ...
    try:
        ...
        print("  ✓ 旧 Runtime 実行ロールが見つかりません（OK）")
    except Exception as e:
    ...
    print("  ✓ 保持されています（put_parameter() は上書きをインテリジェントに処理します）")
        ...
        for lg in log_groups.get('logGroups', []):
            if 'remediation' in lg['logGroupName'].lower():
                try:
                    ...
                    print(f"  ⚠ {lg['logGroupName']} の削除に失敗しました: {e}")

    except logs_client.exceptions.ResourceNotFoundException:
        print("  ✓ ロググループが見つかりません（OK）")
    except Exception as e:
        ...
        for file_path in files_to_delete:
            if os.path.exists(file_path):
                try:
                    ...
                    print(f"  ⚠ {os.path.basename(file_path)} の削除に失敗しました: {e}")
        ...
        for pycache in pycache_paths:
            if '__pycache__' in pycache and os.path.isdir(pycache):
                try:
                    ...
                    print(f"  ✓ 削除しました: __pycache__")
                except Exception as e:
                    print(f"  ⚠ __pycache__ の削除に失敗しました: {e}")

        if deleted_count == 0:
            print("  ✓ ローカルアーティファクトが見つかりません（OK）")
    ...
    try:
        ...
        print(f"Parameter Store でバケット名が見つかりました: {bucket_name}")
        ...
        print(f"バケットを削除しています: {bucket_name}")
    ...
    print("\n" + "=" * 70)
    print("✅ Lab 03 クリーンアップ完了")
    print("\nセクション 1 から Lab 03 を再実行できます")
...
if __name__ == "__main__":
    ...
    print("  • Secrets Manager シークレット（m2m 認証情報）")
