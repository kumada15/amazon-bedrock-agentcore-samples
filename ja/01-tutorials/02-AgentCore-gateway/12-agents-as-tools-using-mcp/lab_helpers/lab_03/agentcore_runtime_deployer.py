"""
Lab 03: AgentCore Runtime デプロイメントヘルパー

Strands 修復エージェントと AgentCore Code Interpreter を Amazon Bedrock AgentCore Runtime にデプロイする。

機能:
- Runtime 実行用の IAM ロール作成
- エージェントコードのパッケージング（Strands + Code Interpreter）
- bedrock-agentcore-starter-toolkit による Runtime デプロイ
- Parameter Store への設定保存
- デプロイメントライフサイクル管理（作成、更新、削除）
- Lab-02 Gateway との統合（オプション）
...
class AgentCoreRuntimeDeployer:
    """Strands 修復エージェントを AgentCore Runtime にデプロイするためのヘルパー"""
    ...
    def __init__(
        ...
        """
        AWS クライアントと設定でデプロイヤーを初期化する。

        Args:
            region: AWS リージョン（デフォルト: us-west-2）
            prefix: リソース命名プレフィックス（デフォルト: aiml301）
            runtime_name: デプロイされる Runtime の名前（デフォルト: aiml301-remediation-runtime）
            verbose: 詳細ロギングを有効化
        """
        self.region = region
        self.prefix = prefix
    ...
    def create_runtime_iam_role(self) -> Dict:
        """
        AgentCore Runtime 実行用の IAM ロールを作成する。

        ロールが許可する内容:
        - Runtime サービスによるロールの引き受け
        - CloudWatch ロギング
        - ECR イメージアクセス
        - Bedrock モデル呼び出し（Code Interpreter 用）
    ...
    def package_agent_code(
        ...
        """
        デプロイ用に Strands 修復エージェントコードをパッケージ化する。
    ...
    def deploy_runtime(
        ...
        """
        Strands エージェントを AgentCore Runtime にデプロイする。

        Args:
            agent_code: エージェント Python コード（文字列）
        ...
        Returns:
            デプロイメント情報を含む Dict（Runtime ID、ARN、エンドポイントなど）
        """
...
def store_runtime_configuration(runtime_arn: str, runtime_id: str = None, region: str = "us-west-2", prefix: str = "aiml301_sre_agentcore") -> None:
    """セッション間の永続化のために Runtime 設定を Parameter Store に保存する"""
