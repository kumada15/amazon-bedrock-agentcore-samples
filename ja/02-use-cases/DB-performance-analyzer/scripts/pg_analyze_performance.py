...
def analyze_query_complexity(query):
    """
    クエリの複雑度と潜在的なリソース影響を分析する

    Args:
        query (str): 分析する SQL クエリ

    Returns:
        dict: 複雑度メトリクス

    Raises:
        QueryComplexityError: クエリが複雑すぎる場合
    """
...
def validate_and_execute_queries(secret_name, query, max_rows=20,
    ...
    Args:
        secret_name: データベース認証情報を含むシークレット名
        query: 実行する SQL クエリ
        max_rows: 返す最大行数
        max_statements: 最大ステートメント数
        max_total_rows: 合計最大行数
        max_complexity: 最大複雑度スコア

    Returns:
        dict: クエリ結果とパフォーマンスメトリクス
    """
    response = {
        'results': [],
    ...
    try:
        ...
        error_msg = str(e)
        logger.warning(f"クエリ制限エラー: {error_msg}")
        raise ValueError(error_msg)

    except psycopg2.Error as pe:
        error_msg = f"Database error: {str(pe)}"
        logger.error(f"データベースエラー: {str(pe)}")
...
def extract_database_object_ddl(secret_name, object_type, object_name=None, object_schema=None):
    """
    データベースオブジェクトの DDL と説明を抽出する

    Args:
        secret_name (str): データベース認証情報を含むシークレット名
        object_type (str): データベースオブジェクトのタイプ（'table', 'view', 'function', 'procedure' など）
        object_name (str, optional): 検索するオブジェクト名
        object_schema (str, optional): フィルタリングするスキーマ名

    Returns:
        list: オブジェクト情報を含む辞書のリスト
        str: オブジェクトが見つからない場合のエラーメッセージ
    """
    try:
        # Input validation
        ...
        with conn.cursor() as cur:
            try:
                # Debug information
                print(f"\n{object_type_lower} のクエリを実行中")
                print(f"オブジェクト名: {object_name}")
                print(f"スキーマ: {object_schema}")
                ...
                rows = cur.fetchall()
                if not rows:
                    print("クエリから行が返されませんでした")
                    return "一致するオブジェクトが見つかりません"
                ...
                for row in rows:
                    ...
                    print(f"\n処理完了 {object_type_lower}: {result.get('object_name', '不明')}")
        ...
        print(f"\n{len(results)} 件のオブジェクトを正常に取得しました")
        return results

    except Exception as e:
        error_msg = f"データベースオブジェクト DDL の抽出に失敗: {str(e)}"
        print(f"\nエラー: {error_msg}")
        raise
...
def generate_object_explanation(obj_info):
    """
    データベースオブジェクトの人間が読める説明を生成する

    Args:
        obj_info (dict): オブジェクト情報を含む辞書
...
def clean_query_for_explain(query):
    """
    クエリから既存の EXPLAIN または EXPLAIN ANALYZE キーワードを削除する
...
def analyze_query_performance(secret_name, query_or_object_name, parameters=None, object_type=None):
    """
    クエリパフォーマンスを分析し、最適化の推奨事項を提供する

    Args:
        secret_name: データベース認証情報を含むシークレット
        query_or_object_name: 分析する SQL クエリ文字列またはオブジェクト名
        parameters: オプション。パラメータ化されたクエリのパラメータ値のリスト
        object_type: オプション。指定された場合、データベースオブジェクトから定義を取得
    """
    conn = connect_to_db(secret_name)
    try:
        with conn.cursor() as cur:
...
def analyze_execution_plan(actual_plan, estimated_plan, is_generic_plan):
    """
    実行計画を分析し、詳細な説明と推奨事項を提供する

    Args:
        actual_plan: 実際の実行計画
        estimated_plan: 推定実行計画
        is_generic_plan: 汎用計画かどうか
...
def analyze_filter_condition(filter_condition, analysis):
    """
    潜在的な最適化機会のためにフィルタ条件を分析する

    Args:
        filter_condition: 分析するフィルタ条件
        analysis: 分析結果を格納する辞書
    """
...
def identify_performance_issues(actual_node, estimated_node, analysis, is_generic_plan):
    """
    実際の計画と推定計画を比較してパフォーマンス問題を特定する

    Args:
        actual_node: 実際のノード
        estimated_node: 推定ノード
        analysis: 分析結果を格納する辞書
        is_generic_plan: 汎用計画かどうか
    """
    if not is_generic_plan:
...
def generate_recommendations(analysis):
    """
    特定された問題に基づいて具体的な推奨事項を生成する

    Args:
        analysis: 問題が特定された分析辞書
    """
...
def monitor_query_performance(query, start_time, rows_returned):
    """
    クエリパフォーマンスを監視し、必要に応じて分析を提案する

    Args:
        query (str): 実行された SQL クエリ
        start_time (float): クエリ開始タイムスタンプ
        rows_returned (int): 返された行数

    Returns:
        dict: パフォーマンスメトリクスと分析の提案
    """
    ...
    if metrics['needs_analysis']:
        ...
        logger.warning(f"遅いクエリを検出しました: {query}")
    else:
...
def validate_query(query):
    """
    セキュリティ上の懸念のためにクエリを検証し、ステートメントに分割する

    Args:
        query (str): 検証する SQL クエリ

    Returns:
        list: 検証済みステートメントのリスト

    Raises:
        ValueError: クエリに禁止された操作が含まれている場合
    """
    if not query or not isinstance(query, str):
        raise ValueError("Query must be a non-empty string")

    def is_within_quotes(text, position):
        """テキスト内の位置が引用符の中にあるかを確認する"""
        single_quotes = False
        double_quotes = False
    ...
    def split_statements(query_text):
        """クエリを個別のステートメントに分割する（引用符とコメントを考慮）"""
        statements = []
        current_stmt = []
        i = 0
        comment_block = False
...
def execute_read_query(secret_name, query, max_rows=20):
    """
    読み取り専用クエリを安全に実行し、監視付きで結果を返す

    Args:
        secret_name (str): データベース認証情報を含むシークレット
        query (str): 実行する SQL クエリ
        max_rows (int): 返す最大行数（SELECT クエリのみ）
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            for stmt_index, stmt in enumerate(statements, 1):
                ...
                try:
                    ...
                    logger.error(f"クエリ実行エラー: {final_query}")
                    logger.error(f"エラー詳細: {str(pe)}")
                    raise
        ...
        logger.error(f"クエリ検証に失敗しました: {str(ve)}")
        raise ValueError(error_msg)
...
def execute_enhanced_query_diagnostics(secret_name, query):
    """
    runbooks.py に基づいた拡張クエリ診断を実行する
    実行計画、パフォーマンスメトリクス、最適化の提案を含む包括的な分析を提供する
...
def execute_performance_insights_analysis(secret_name):
    """
    AWS Performance Insights に類似したパフォーマンス分析を実行する
    runbooks.py の診断パターンに基づく
...
def lambda_handler(event, context):
    try:
        ...
        print(f"lambda_handler でエラー: {str(e)}")  # デバッグ用
        return {
            "functionResponse": {
