"""
Bedrock-AgentCore Code Interpreter を使用した動的リサーチエージェント
シンプルなアーキテクチャと堅牢なエラーハンドリング付き
"""
...
class ResearchAgent:
    ...
    def _setup_working_environment(self):
        """詳細なフィードバック付きで Code Interpreter の作業環境をセットアップする"""
        setup_code = """
import os
import sys
import matplotlib
...
try:
    ...
    print("✓ ファイル書き込みテストに成功しました")
except Exception as e:
    print(f"テストファイル書き込みエラー: {e}")
...
"""
    ...
    def _extract_code_block(self, text: str) -> str:
        """Markdown コードブロックを含む可能性のあるテキストからコードを抽出する"""
        if "```python" in text:
...
Return ONLY the Python code, no explanations."""
    ...
    def create_workflow(self) -> StateGraph:
        """すべてのステップを試行するシンプルな線形ワークフローを作成する"""
...
print(f"{n_customers} 人の顧客から {len(df)} 件の購入データを作成しました")
print(f"データを data/research_data.csv に保存しました")
print(f"顧客統計を data/customer_stats.csv に保存しました")
print(f"基本的な可視化を visualizations/satisfaction_distribution.png に保存しました")
...
"""
    ...
    def generate_insights(self, state: AgentState) -> AgentState:
        """前のステップの成功に関係なく、インサイトを含む最終レポートを生成する"""
...
async def run_research(query: str):
    ...
    with ResearchAgent() as agent:
        ...
        console.print(f"完了: {', '.join(final_state['completed_tasks'])}")
