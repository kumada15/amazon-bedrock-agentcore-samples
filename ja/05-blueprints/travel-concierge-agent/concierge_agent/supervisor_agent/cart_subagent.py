...
- 購入フロー（カードチェック付き2ステップ）:
  ステップ 1: ユーザーが購入意図を示したとき（「買う」「チェックアウト」「購入」など）:
    ...
    * ユーザーがカードを持っている場合（has_card: true）:
      ...
      - 1泊あたりの購入がある場合は、ホテル3泊で1泊800ドルなら 800*3 = 2400 のように要約する
      - ユーザーに概要を提示し、確認を求める
...
async def cart_manager(query: str, user_id: str = "", session_id: str = ""):
    ...
    利用可能なツール:
    - get_cart: カートの内容を表示（user_id）
    - add_to_cart: 商品を追加（user_id、asin/title/price を含む items リスト）
    - add_hotel_to_cart: ホテルを追加（user_id、title/price/hotel_id/city_code を含む hotels リスト）
    - add_flight_to_cart: フライトを追加（user_id、title/price/flight_id/origin/destination/departure_date を含む flights リスト）
    - remove_from_cart: アイテムを削除（user_id、identifiers リスト、item_type: product/hotel/flight）
    - clear_cart: カートを空にする（user_id）
    - check_user_has_payment_card: 支払い方法を確認（user_id）
    - request_purchase_confirmation: チェックアウト前の概要（user_id）
    - confirm_purchase: 確認後に購入を実行（user_id）
    - onboard_card: 支払いカードを追加（user_id、カード詳細）
    - get_visa_iframe_config: セキュアなカード入力設定を取得（user_id）

    ここにルーティング:
    - カートを表示: 「カートの中身は?」「カートを見せて」
    - アイテムを追加: 「このホテルをカートに追加」「フライトを追加」
    - アイテムを削除: 「ホテルを削除」「フライトを外す」
