...
API キーにより追加機能が有効になりますが、基本機能には必須ではありません。アプリケーションは基本機能でそのまま動作します。

#### SerpAPI（商品検索）

**サインアップ**: https://serpapi.com/
- 旅行検索機能を有効化
- SSM Parameter Store に保存: `/concierge-agent/travel/serp-api-key`

#### Visa API（決済連携）

**デフォルト**: アプリケーションはデフォルトで**モックモード**を使用 - Visa 認証情報不要！

**実際の Visa 連携の場合**:
- [Visa オンボーディングプロセス](visa-documentation/visa-payment-integration-guide.md#onboarding-process)に従ってください
- 詳細なセットアップについては以下の [Visa 決済設定](#visa-決済設定) セクションを参照
...
Cart と Travel MCP ランタイムを別々のスタックとしてデプロイします。
...
**デプロイされるもの:**
- **CartStack** - DynamoDB と Visa Secrets Manager アクセスを持つ旅行カート管理
- **TravelStack** - SerpAPI 連携を持つ旅行検索ツール
- **ItineraryStack** - 旅行用の旅程
- OAuth 認証を使用し、Cognito と連携
...
### MCP サーバーコードの更新
```bash
# concierge_agent/mcp_cart_tools/ または mcp_travel_tools/ に変更を加える
