"""
Lab 02: ZIP ベースの Lambda デプロイパッケージャー

Docker ベースのデプロイを純粋な Python ZIP パッケージングに置き換え。
SageMaker VPC モードでネイティブに動作（Docker デーモン不要）。

関数:
- create_deployment_package()      # 依存関係を含む ZIP を作成
- upload_package_to_s3()           # S3 にアップロード
- create_lambda_function_from_zip() # ZIP から Lambda をデプロイ
- get_package_info()                # サイズと内容を検証
- setup_s3_bucket()                 # S3 バケットを作成/検証
"""
...
def validate_requirements(build_dir: str) -> bool:
    """ビルドディレクトリに requirements.txt が存在することを検証"""
...
def install_dependencies(
    ...
    """
    pip を使用して lib/ ディレクトリに依存関係をインストール

    Args:
        build_dir: 依存関係がインストールされるディレクトリ
        requirements_content: requirements.txt の内容

    Returns:
        (success: bool, stats: インストール情報を含む辞書)
    """
...
def create_lambda_zip(
    ...
    Args:
        build_dir: ソースビルドディレクトリ
        handler_code: app.py 用の Python コード
        output_zip: 出力 ZIP ファイルパス

    Returns:
        (success: bool, stats: 辞書)
    """
...
def create_deployment_package(
    ...
    """
    完全なワークフロー: 依存関係を含むデプロイパッケージを作成

    Args:
        handler_code: Lambda ハンドラー（app.py）用の Python コード
        requirements_content: pip requirements テキスト
        build_dir: ビルドディレクトリ名

    Returns:
        パッケージ情報またはエラー詳細を含む辞書
    """
    print("=" * 70)
...
def setup_s3_bucket(bucket_name: str, region_name: Optional[str] = None) -> Dict:
    """
    Lambda デプロイパッケージ用の S3 バケットを作成または検証
...
def upload_package_to_s3(
    ...
    """
    ZIP パッケージを S3 にアップロード

    Args:
        zip_path: ローカル ZIP ファイルパス
        s3_bucket: S3 バケット名
        s3_key: S3 オブジェクトキー
        region_name: AWS リージョン

    Returns:
        S3 URI とアップロード情報を含む辞書
    """
    if region_name is None:
        region_name = AWS_REGION
...
def create_lambda_function_from_zip(
    ...
    """
    ZIP パッケージから Lambda 関数を作成または更新

    Args:
        function_name: Lambda 関数名
        zip_path: ローカル ZIP ファイルパス（直接アップロード用、<50MB）
        s3_uri: S3 URI（S3 アップロード用、>50MB）。フォーマット: s3://bucket/key
    ...
    try:
        ...
        print(f"アップロード方法: {upload_method}")
...
def setup_lambda_zip_deployment(
    ...
    """
    完全なワークフロー: パッケージ作成 → （オプション）S3 アップロード → Lambda デプロイ

    Args:
        handler_code: Lambda ハンドラー用の Python コード
        requirements_content: pip requirements テキスト
        region_name: AWS リージョン
