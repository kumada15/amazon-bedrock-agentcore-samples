                ...
                try:
                    ...
                    if any(
                        ...
                        if attempt < max_retries - 1:
                            print(
                                f"試行 {attempt + 1} でエラーを検出しました。再試行中..."
                            )
                            time.sleep(2)
                            continue
                        else:
                    ...
                    if attempt < max_retries - 1:
                        print(f"試行 {attempt + 1} が失敗しました。再試行中...")
                        time.sleep(2)
                        continue
                    else:
                        ...
                        if fallback_url:
                            print(
                                f"{news_source} へのすべての試行が失敗しました。フォールバックを試行中: {fallback_url}"
                            )
...
def search_news(query: str, news_source: str = "bloomberg") -> str:
    """
    任意のニュースソースからビジネスニュースを検索する。

    Args:
        query (str): 検索クエリ
        news_source (str): ニュースソースのオプション:
            - bloomberg (最も信頼性が高い)
            - yahoo finance (非常に信頼性が高い)
            - marketwatch (信頼性が高い)
            - reuters (問題が発生する可能性あり)
            - cnbc (サーバーの問題が発生する可能性あり)
            - wsj (ペイウォールあり)
            - financial times (ペイウォールあり)
            - seeking alpha (信頼性が高い)
    ...
    try:
            ...
            if any(indicator in result.lower() for indicator in error_indicators):
                ...
                for fallback_source in reliable_sources:
                    if fallback_source != news_source.lower():
                        try:
                            print(
                                f"フォールバック: {news_source} の代わりに {fallback_source} を試行中"
                            )
                            return search_news_with_browser(p, query, fallback_source)
                        except Exception as fallback_error:
                            logger.warning(
                                f"フォールバックソース {fallback_source} が失敗しました: {fallback_error}"
                            )
                            continue
        ...
        for fallback_source in reliable_sources:
            if fallback_source != news_source.lower():
                try:
                    print(
                        f"{news_source} でエラーが発生しました。フォールバックとして {fallback_source} を試行中"
                    )
                    with sync_playwright() as p:
                        return search_news_with_browser(p, query, fallback_source)
                except Exception as final_fallback_error:
                    logger.warning(
                        f"最終フォールバックソース {fallback_source} が失敗しました: {final_fallback_error}"
                    )
                    continue
