...
- **IAM ロール** - Bedrock モデル、CloudWatch、SSM パラメータの権限

**デプロイされるスタック**:
- **TravelStack** - 旅行計画ツール (天気、目的地、フライト、ホテル)
- **CartStack** - ショッピングカートと Visa 決済連携
- **ItineraryStack** - 旅程管理ツール
...
│   │   ├── travel-stack.ts   # 旅行ツール
│   │   ├── cart-stack.ts     # カート & 決済
│   │   ├── itinerary-stack.ts # 旅程管理
│   │   └── internet-search-stack.ts # Web 検索
│   ├── cdk.json
│   └── package.json
│
...
例: `TravelStack-default-RuntimeArn`
...
- `ITINERARY_TABLE_NAME` - DynamoDB テーブル名
...
**エラー**: `Export ConciergeAgent-default-Auth-UserPoolId not found`
