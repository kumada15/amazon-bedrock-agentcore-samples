...
def provision_token(
    ...
    logger.info("=" * 80)
    logger.info("PAN EnrollmentID からトークンのプロビジョニングを開始")
    logger.info("=" * 80)

    if not x_request_id:
        logger.warning(
            "x-request-id が提供されていません - VPP セッションの継続性が失われる可能性があります!"
        )
        x_request_id = str(uuid.uuid4())
    ...
    if browser_data:
        logger.info("Visa iframe から実際のブラウザデータを使用中")
        logger.info(f"ブラウザデータキー: {list(browser_data.keys())}")
        ...
        logger.warning("ブラウザデータが提供されていません - ダミーデバイスデータを使用中")
        _risk_data = {
            "deviceFingerprint": {
    ...
    try:
        ...
        logger.info("\n" + "=" * 80)
        logger.info("トークンのプロビジョニングが正常に完了しました")
        logger.info("=" * 80)
        ...
        logger.error("\n" + "=" * 80)
        logger.error("トークンのプロビジョニングに失敗しました")
        logger.error("=" * 80)
        logger.error(f"エラー: {str(e)}")
        if hasattr(e, "response") and e.response is not None:
            logger.error(f"レスポンスステータスコード: {e.response.status_code}")
...
def get_secure_token(api_key, client_app_id, headless=False):
    ...
    """
    logger.info("=" * 80)
    logger.info("Visa Secure Token を取得中")
    logger.info("=" * 80)
    ...
    if result and "secureToken" in result:
        ...
        logger.info(f"Secure Token を正常に取得しました: {secure_token[:60]}...")
        return secure_token
    else:
        logger.error("Secure Token の取得に失敗しました")
        return None
