...
print_warning "  - Memory リソース（すべての会話履歴を含む）"
print_warning "  - Memory Initializer Lambda"
print_warning "  - 2つの S3 バケット（ソースコード + 結果ストレージ）"
print_warning "  - 1つの ECR リポジトリ（すべてのイメージを含む）"
print_warning "  - 1つの CodeBuild プロジェクト"
print_warning "  - IAM ロールとポリシー（エージェント + ツール + Lambda）"
...
if [ "$RUNTIME_COUNT" -eq 0 ]; then
    print_success "Weather Agent Runtime がクリーンアップされました"
else
...
if [ "$MEMORY_COUNT" -eq 0 ]; then
    print_success "Memory リソースがクリーンアップされました"
else
    print_warning "'$STACK_NAME' に一致する Memory リソースが $MEMORY_COUNT 個見つかりました"
    print_info "手動でクリーンアップが必要な場合があります"
fi
...
if [ "$TOOL_COUNT" -eq 0 ]; then
    print_success "ツール（Browser + Code Interpreter）がクリーンアップされました"
else
    print_warning "'$STACK_NAME' に一致するツールが $TOOL_COUNT 個見つかりました"
    print_info "手動でクリーンアップが必要な場合があります"
fi
...
if [ "$LAMBDA_COUNT" -eq 0 ]; then
    print_success "Lambda 関数がクリーンアップされました"
else
    print_warning "'$STACK_NAME' に一致する Lambda 関数が $LAMBDA_COUNT 個見つかりました"
    print_info "手動でクリーンアップが必要な場合があります"
fi
...
if [ "$S3_BUCKETS" -eq 0 ]; then
    print_success "S3 バケットがクリーンアップされました（ソース + 結果）"
else
...
echo ""
print_info "2. Memory - Memory リソースが残っていないこと"
print_info "   確認方法: aws bedrock-agentcore list-memories --region $AWS_REGION"
echo ""
print_info "3. Tools - Browser/Code Interpreter ツールが残っていないこと"
print_info "   確認方法: aws bedrock-agentcore list-browser-customs --region $AWS_REGION"
echo ""
print_info "4. Lambda - Memory Initializer 関数が残っていないこと"
print_info "   https://console.aws.amazon.com/lambda/home?region=$AWS_REGION#/functions"
echo ""
print_info "5. S3 - バケットが残っていないこと（ソース + 結果）"
print_info "   https://console.aws.amazon.com/s3/buckets?region=$AWS_REGION"
echo ""
print_info "6. ECR - リポジトリが残っていないこと"
print_info "   https://console.aws.amazon.com/ecr/repositories?region=$AWS_REGION"
echo ""
print_info "7. CodeBuild - プロジェクトが残っていないこと"
print_info "   https://console.aws.amazon.com/codesuite/codebuild/projects?region=$AWS_REGION"
echo ""
print_info "8. CloudWatch Logs - 孤立したロググループがないか確認"
