...
Gateway インフラストラクチャを作成し、修復 Runtime をターゲットとして登録します。

Lab-02 パターン + Gateway-to-Runtime M2M 認証に基づいています。

機能:
- 適切な信頼ポリシーを持つ Gateway サービスロールを作成
- IAM 認証付きの AgentCore Gateway を作成
- 修復 Runtime を MCP ターゲットとして登録
- M2M OAuth2 認証をサポート（オプション）
...
class AgentCoreGatewaySetup:
    """Runtime ターゲット付き AgentCore Gateway のセットアップヘルパー"""

    def __init__(self, region: str = REGION, prefix: str = PREFIX, verbose: bool = True):
        """
        Gateway セットアップヘルパーを初期化。

        Args:
            region: AWS リージョン
            prefix: リソース命名プレフィックス
            verbose: ロギングを有効化
        """
        self.region = region
        self.prefix = prefix
    ...
    def create_gateway_service_role(self) -> Dict:
        """
        Gateway が Runtime ターゲットを呼び出すための IAM サービスロールを作成。

        Gateway に必要な権限:
        1. Runtime ターゲットの呼び出し
        2. CloudWatch ログへのアクセス
        3. AgentCore リソースの管理
        4. OAuth 資格情報へのアクセス（M2M 認証用）

        Returns:
            ロール ARN、ロール名、メタデータを含む Dict
        """
    ...
    def create_gateway(
        ...
        Args:
            ...
            role_arn: サービスロール ARN（指定されない場合は Parameter Store から取得）
            protocol_type: Gateway プロトコル（MCP、HTTP など）
            authorizer_type: インバウンド認証タイプ（AWS_IAM、CUSTOM_JWT）
            authorizer_configuration: JWT オーソライザー設定（CUSTOM_JWT の場合必須）
                フォーマット: {"customJWTAuthorizer": {"discoveryUrl": "...", "allowedClients": [...]}}

        Returns:
            Gateway ID、URL、メタデータを含む Dict
        """
    ...
    def register_runtime_target(
        ...
        """
        Runtime を Gateway のターゲットとして登録。

        Args:
            gateway_id: Gateway 識別子
            runtime_arn: ターゲットとして登録する Runtime ARN
            target_name: ターゲットの名前
            tool_schema: ツールスキーマ定義（オプション）
            credentials_type: 資格情報プロバイダータイプ（GATEWAY_IAM_ROLE、OAUTH2）

        Returns:
            ターゲット ID とメタデータを含む Dict
        """
    ...
    def list_gateway_targets(self, gateway_id: str) -> List[Dict]:
        """Gateway に登録されているすべてのターゲットを一覧表示"""
        try:
    ...
    def get_stored_config(self) -> Dict:
        """Parameter Store から保存された Gateway と Runtime ターゲット設定を取得"""
        try:
            config = {}
    ...
    def cleanup(self, force: bool = False) -> bool:
        """
        Lab-03 Gateway リソースをクリーンアップ。

        Args:
            force: 確認なしで強制削除

        Returns:
            クリーンアップが成功した場合は True
        """
