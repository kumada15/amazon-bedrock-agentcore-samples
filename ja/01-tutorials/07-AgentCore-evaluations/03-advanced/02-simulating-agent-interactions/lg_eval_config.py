...
CUSTOM_EVALUATOR_CONFIG = {
    "llmAsAJudge": {
        ...
        "instructions": "あなたは Web 検索エージェントのレスポンス品質を評価する客観的な審査員です。以下を評価してください: (1) エージェントがユーザーのクエリに対して適切な Web 検索を実行したか、(2) 使用された検索結果の関連性と品質、(3) エージェントが検索結果からの情報をどれだけうまく統合したか、(4) ソースが URL とともに適切に帰属されているか、(5) 情報が最新で正確に見えるか。会話のコンテキストを考慮し、対象のターンを評価してください。重要: 検索品質、情報統合、ソース帰属に焦点を当ててください。検索の遅延や API の制限によるペナルティは与えないでください。# 会話コンテキスト: ## 以前のターン: {context} ## 評価対象のターン: {assistant_turn}",
    }
}
