...
def get_zip_size(zip_path: str) -> int:
    """圧縮済み ZIP ファイルのサイズをバイト単位で取得"""
    return os.path.getsize(zip_path)


def format_size(size_bytes: int) -> str:
    """バイトを人間が読みやすいサイズにフォーマット"""
...
def validate_requirements(build_dir: str) -> bool:
    """ビルドディレクトリに requirements.txt が存在することを確認"""
    req_file = os.path.join(build_dir, "requirements.txt")
    if not os.path.exists(req_file):
        print(f"❌ requirements.txt が {build_dir} に見つかりません")
        return False
    return True
...
def install_dependencies(
    ...
    print(f"✓ requirements.txt を書き込みました ({len(requirements_content)} バイト)")
    ...
    print(f"✓ lib/ にインストール中...")
    print(f"  ターゲット: Python 3.11 Linux x86_64（Lambda ランタイム）")
    try:
        ...
        print(f"✓ {len(installed_packages)} 個のパッケージをインストールしました")
        print(f"  lib/ サイズ: {format_size(lib_size)}")
        ...
        print(f"❌ 5分後にインストールがタイムアウトしました")
        return False, {}
    except subprocess.CalledProcessError as e:
        print(f"❌ インストールに失敗しました: {e.stderr}")
        return False, {}
    except Exception as e:
        print(f"❌ インストール中に予期しないエラーが発生しました: {e}")
        return False, {}
...
def create_lambda_zip(
    ...
    """
    print(f"\n📦 Lambda ZIP パッケージを作成中...")

    # Write app.py
    app_py = os.path.join(build_dir, "app.py")
    with open(app_py, "w") as f:
        f.write(handler_code)
    print(f"✓ app.py を書き込みました ({len(handler_code)} バイト)")
    ...
    try:
        with zipfile.ZipFile(output_zip, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            if os.path.exists(lib_dir):
                for root, dirs, files in os.walk(lib_dir):
                    for file in files:
                        ...
                        total_files += 1
                print(f"✓ lib/ から {total_files} 個のファイルを追加しました")
            ...
            if os.path.exists(lab_helpers_dir):
                ...
                for root, dirs, files in os.walk(lab_helpers_dir):
                    for file in files:
                        ...
                        zf.write(file_path, arcname)
                        total_files += 1
                print(f"✓ lab_helpers/ から {total_files - helpers_start} 個のファイルを追加しました")
            ...
            total_files += 2
            print(f"✓ ルートに app.py と requirements.txt を追加しました")

        zip_size = get_zip_size(output_zip)

        print(f"✓ ZIP を作成しました: {output_zip}")
        print(f"  圧縮サイズ: {format_size(zip_size)}")
...
def create_deployment_package(
    ...
    """
    print("=" * 70)
    print("LAMBDA ZIP デプロイメントパッケージを作成中")
    print("=" * 70)

    # Cleanup old build if exists
    if os.path.exists(build_dir):
        print(f"\n🧹 既存のビルドディレクトリをクリーンアップ中...")
        shutil.rmtree(build_dir)

    os.makedirs(build_dir, exist_ok=True)
    print(f"✓ ビルドディレクトリを作成しました: {build_dir}")
    ...
    if os.path.exists(lab_helpers_src):
        ...
        print(f"\n📂 lab_helpers をビルドディレクトリにコピー中...")
        shutil.copytree(lab_helpers_src, lab_helpers_dest, ignore=shutil.ignore_patterns('__pycache__', '*.pyc', '.pytest_cache'))
        print(f"✓ lab_helpers/ を {build_dir}/lab_helpers/ にコピーしました")
    else:
        print(f"⚠️  lab_helpers ディレクトリがリポジトリルートに見つかりません")
    ...
    print(f"{size_status} 圧縮サイズ: {format_size(zip_size)}（50 MB 直接 / 250 MB S3 制限）")
    print(f"✓ 非圧縮サイズ: {format_size(uncompressed_size)}（250 MB 制限）")
    print(f"✓ デプロイ方式: {upload_method}")
...
def setup_s3_bucket(bucket_name: str, region_name: Optional[str] = None) -> Dict:
    ...
    print(f"\n📦 デプロイメントパッケージ用の S3 バケットをセットアップ中...")
    ...
    try:
        ...
        if is_not_found:
            # Create bucket
            print(f"✓ S3 バケットを作成中: {bucket_name}")
            ...
            try:
                ...
                bucket_arn = f"arn:aws:s3:::{bucket_name}"
                print(f"✓ バケットを作成しました: {bucket_arn}")
                ...
                if any(err in create_error_str for err in ["BucketAlreadyExists", "BucketAlreadyOwnedByYou"]):
                    print(f"✓ S3 バケットが存在します（同時作成）: {bucket_name}")
                    bucket_arn = f"arn:aws:s3:::{bucket_name}"
                else:
                    print(f"❌ バケット作成エラー: {create_error}")
                    raise
        else:
            # Other error - re-raise
            print(f"❌ バケットの確認/セットアップエラー: {e}")
            raise
...
def upload_package_to_s3(
    ...
    print(f"\n📤 S3 にパッケージをアップロード中...")
    print(f"   ローカルファイル: {zip_path} ({format_size(zip_size)})")
    print(f"   送信先: s3://{s3_bucket}/{s3_key}")
...
def create_lambda_function_from_zip(
    ...
    try:
        ...
        try:
            ...
            print(f"  関数がアクティブになるのを待機中...")
            waiter = lambda_client.get_waiter('function_active')
            waiter.wait(FunctionName=function_name)

            print(f"✓ 関数を作成しアクティブになりました")
        ...
        print(f"\n" + "=" * 70)
        print(f"✅ LAMBDA デプロイ成功")
        print(f"=" * 70)
        print(f"関数: {config['FunctionName']}")
        ...
        print(f"状態: {config['State']}")
        print(f"アップロード方式: {upload_method}")
...
def setup_lambda_zip_deployment(
    ...
    try:
        ...
        print(f"❌ Parameter Store から Lambda ロール ARN を取得できませんでした")
    ...
    print(f"\n📝 Lambda ARN を Parameter Store に保存中...")
    put_parameter(
