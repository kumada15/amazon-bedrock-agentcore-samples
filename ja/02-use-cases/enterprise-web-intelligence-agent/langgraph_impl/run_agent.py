#!/usr/bin/env python3
"""AWS に焦点を当てた例で競合インテリジェンスエージェントを実行する。"""
...
def show_competitors_table(competitors: List[Dict]):
    ...
    table.add_column("分析項目", style="green")
...
async def view_replay(recording_config: Any, config: AgentConfig):
    ...
    try:
        ...
        if isinstance(recording_config, dict):
            ...
            console.print("[yellow]⚠️ レガシー S3 パス形式を使用中[/yellow]")
...
async def choose_session_to_replay(results: Dict):
    """複数のセッションがある場合にユーザーがリプレイするセッションを選択できるようにする。"""
    if not results.get("parallel_sessions"):
        # Only one session, use the default
        return None

    console.print("\n[bold cyan]複数のブラウザセッションが利用可能です:[/bold cyan]")
    console.print("再生する競合セッションを選択してください:")
    ...
    console.print(f"[cyan]選択されました: {selected_session.get('name', '不明')}[/cyan]")
...
async def main():
    ...
    if not config.validate():
        console.print("[red]❌ 設定の検証に失敗しました[/red]")
        console.print("以下の環境変数を設定してください:")
    ...
    console.print(f"  S3 プレフィックス: {config.s3_prefix}")
    ...
    if len(competitors) > 1:
        ...
        if parallel_mode:
            console.print("[yellow]注意: 並列処理ではライブビューの可視性が制限されます[/yellow]")
            console.print("[yellow]⚠️ 並列モードではセッションリプレイは利用できません[/yellow]")
    ...
    try:
        ...
        console.print("[dim]上のブラウザビューアリンクを開いて自動化を確認してください！[/dim]")
        ...
        if results["success"]:
            ...
            if results.get("apis_discovered"):
                console.print("\n[bold]検出された API エンドポイント:[/bold]")
                for api in results["apis_discovered"][:5]:  # Show first 5
                    console.print(f"  • {api['url'][:80]}...")
                if len(results["apis_discovered"]) > 5:
                    console.print(f"  ... 他 {len(results['apis_discovered']) - 5} 件")
            
            # Save session info
            if results.get("session_id"):
                console.print(f"\n[cyan]💾 セッションを保存しました ID: {results['session_id']}[/cyan]")
                console.print("[dim]この ID を使用して後でこのセッションを再開できます[/dim]")
