...
def lambda_handler(event, context):
    try:
        print(f"受信したイベント: {json.dumps(event, default=str)}")
        ...
        try:
            ...
            print(f"Tavily で検索中: {query}")

            # Use urllib3 for Tavily API
            http = urllib3.PoolManager()

            search_data = {"api_key": api_key, "query": query, "max_results": 3}
...
5) Recovery timeline (if diseased)"""
                    ...
                    print("Tavily 検索結果を使用します")
                    use_tavily = True
                else:
                    raise Exception("Tavily から検索結果がありません")
            else:
                raise Exception(f"Tavily API returned status {response.status}")

        except Exception as e:
            print(f"Tavily が失敗しました: {e}、Nova にフォールバックします")
            use_tavily = False
...
7) Companion planting suggestions"""
        ...
        return {
            "statusCode": 500,
            "body": json.dumps(
                {"error": str(e), "web_search_results": f"Search failed: {str(e)}"}
            ),
        }
