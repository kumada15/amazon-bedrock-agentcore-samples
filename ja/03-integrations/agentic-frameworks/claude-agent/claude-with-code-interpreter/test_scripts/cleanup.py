...
def _terminate_code_int(session_id: str):
    logger.info("*" * 80 + "\n")
    logger.info("コードインタープリターセッションをクリーンアップ中: %s", session_id)
    ...
    logger.info("*" * 80 + "\n")
    logger.info("コードインタープリター終了レスポンス: %s", response)
...
def _stop_runtime_session(agent_arn: str, session_id: str):
    ...
    logger.info("セッションの終了に成功")
    logger.info("リクエスト ID: %s", {response["ResponseMetadata"]["RequestId"]})
...
if __name__ == "__main__":
    ...
    logger.info("Code Interpreter セッションをクリーンアップ中: %s", args.code_int_session_id)
    logger.info("Runtime セッションをクリーンアップ中: %s", args.runtime_session_id)
