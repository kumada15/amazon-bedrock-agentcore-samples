...
def load_hp_config(config_path="cicd/hp_config.json"):
    """JSON ファイルからハイパーパラメータとエージェント設定を読み込む。"""
    try:
...
# Load configuration
print("hp_config.jsonからエージェント設定を読み込み中...")
config = load_hp_config()

# Check if agent_arn exists in the config
if not config.get("agent_arn"):
    print("エラー: hp_config.jsonにagent_arnが見つかりません。")
    print("最初にデプロイステップを実行してください。")
    sys.exit(1)

agent_arn = config["agent_arn"]
print(f"デプロイからのAgent ARNを使用中: {agent_arn}")
...
print(f"\n{'='*80}\nデータセット'{dataset_name}'からの最初の3つのORIGINALアイテム:\n{'='*80}")
...
print(f"\n{'='*80}\nデータセット'{dataset_name}'からの最初の3つのEXPANDEDアイテム:\n{'='*80}")
