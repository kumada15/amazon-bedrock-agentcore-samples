# Strands Agents での評価の実行

## 概要

このチュートリアルでは、[Strands Agents SDK](https://strandsagents.com/) を使用して構築されたエージェントで AgentCore Evaluations を使用する方法を示します。組み込みおよびカスタム評価者を使用して Strands エージェントのパフォーマンスを評価・監視するために、オンデマンドとオンライン評価の両方を実行する方法を学びます。

## 学習内容

- 特定の Strands エージェントトレースに対するオンデマンド評価の実行
- Strands エージェントの継続的な監視のためのオンライン評価のセットアップ
...
- [チュートリアル 00: 前提条件](../../00-prereqs) を完了し、Strands エージェント（`eval_agent_strands.py`）を作成済み
- [チュートリアル 01: カスタム評価者の作成](../../01-creating-custom-evaluators) を完了し、カスタム評価者を作成済み
- Strands エージェントが AgentCore Runtime にデプロイ済み
...
**チュートリアルタイプ:** オンデマンド評価者（組み込みおよびカスタム）を使用した Strands エージェントの評価

**学習内容：**
- デプロイされた Strands エージェントからセッションとトレース情報を取得する方法
...
**チュートリアルタイプ:** オンライン評価者（組み込みおよびカスタム）を使用した Strands エージェントの評価

**学習内容：**
- Strands エージェント用のオンライン評価設定の作成
...
## Strands エージェントアーキテクチャ

これらのチュートリアルで使用される Strands エージェントには以下が含まれます：
...
## Strands エージェントでの評価の仕組み

1. **エージェント呼び出し**: Strands エージェントがユーザーリクエストを処理
...
- オンデマンド評価を使用して特定の Strands エージェントインタラクションを評価
- 本番 Strands エージェントの継続的な品質監視をセットアップ
- 改善領域を特定するための評価結果の分析
...
これらの Strands 固有のチュートリアルを完了した後：
- [LangGraph の例](../02-langgraph/) を探索して、異なるフレームワークでの評価の動作を確認
