...
def run_experiment(
    ...
    - evaluators (list, optional): アイテムレベル評価用の評価関数リスト
    - run_evaluators (list, optional): 実行レベル評価用の評価関数リスト
    ...
    def agent_task(*, item, **kwargs):
        ...
        Parameters:
        - item: 入力と（オプションで）expected_output を含む DatasetItemClient オブジェクト
...
def run_experiment_with_evaluators(
    ...
    def response_quality_evaluator(*, input, output, expected_output, metadata, **kwargs):
        """
        基本的な品質チェック - 応答にエラーインジケータが含まれていないことを確認します。
        """
        if isinstance(output, str):
