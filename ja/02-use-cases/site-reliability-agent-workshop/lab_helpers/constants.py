"""
すべての AWS Systems Manager Parameter Store パスの中央レジストリ
すべてのラボでデプロイと取得ヘルパーによって使用

これにより、パラメータの命名が一貫性があり、発見可能で、バージョン管理されます。
ノートブックやヘルパー関数にハードコードされた値はありません。
"""
...
# Lambda 関数設定（定数仕様）
LAMBDA_CONFIG = {
    "memory_size": 2048,  # MB（Strands エージェント + モデル推論用に 2GB）
    "timeout": 300,       # 秒（Strands エージェントの推論用に 5 分）
...
BEDROCK_CONFIG = {
    ...
    # MODEL_ID は config.py で管理され、デプロイヤーによって必要に応じてインポートされます
}

# ZIP ベースの Lambda デプロイ用 S3 設定
S3_CONFIG = {
...
DEPLOYMENT_METHODS = {
    "docker": {
        "description": "Docker イメージ → ECR → Lambda（従来のアプローチ）",
        "requires": ["docker", "ecr"],
        "vpc_compatible": False,
        "size_limit": "10GB（コンテナ制限）",
    },
    "zip_direct": {
        "description": "ZIP ファイル → Lambda 直接アップロード（<50MB）",
        "requires": ["python", "pip", "aws_cli"],
        "vpc_compatible": True,
        "size_limit": "50 MB（直接アップロード制限）",
    },
    "zip_s3": {
        "description": "ZIP ファイル → S3 → Lambda（推奨）",
        "requires": ["python", "pip", "aws_cli", "s3"],
        "vpc_compatible": True,
        "size_limit": "250 MB（S3 制限）",
    },
}
