...
"""BedrockAgentCore サービス用のエンドポイントユーティリティ。"""
...
class BedrockAgentCoreClient:
    ...
    def __init__(self, region: str):
        ...
        self.logger.debug("リージョン %s 用の Bedrock AgentCore クライアントを初期化しています", region)
    ...
    def create_agent(
        ...
        """新しいエージェントを作成します。"""
        self.logger.info("イメージ URI: %s でエージェント '%s' を作成しています", image_uri, agent_name)
        try:
            ...
            self.logger.info("エージェント '%s' を正常に作成しました。ID: %s, ARN: %s", agent_name, agent_id, agent_arn)
    ...
    def update_agent(
        ...
        """既存のエージェントを更新します。"""
        self.logger.info("イメージ URI: %s でエージェント ID '%s' を更新しています", image_uri, agent_id)
        try:
            # Build parameters dict, only including optional configs when present
            params = {
            ...
            agent_arn = resp["agentRuntimeArn"]
            self.logger.info("エージェント ID '%s' を正常に更新しました。ARN: %s", agent_id, agent_arn)
            return {"id": agent_id, "arn": agent_arn}
        except Exception as e:
...
class HttpBedrockAgentCoreClient:
    ...
    def __init__(self, region: str):
        ...
        self.logger.debug("リージョン %s 用の HTTP Bedrock AgentCore クライアントを初期化しています", region)
    ...
    def invoke_endpoint(
        ...
        try:
            ...
            self.logger.warning("ペイロードを JSON として解析できませんでした。ペイロードオブジェクトにラップします")
