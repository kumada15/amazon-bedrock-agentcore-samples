...
def create_agentcore_role(agent_name):
    ...
    try:
        ...
        inline_policies = iam_client.list_role_policies(RoleName=agentcore_role_name)
...
def create_lambda_role(lambda_role_name):
    ...
    try:
        ...
        print(f"IAM ロールを作成しました: {lambda_role_name}")
        iam_client.put_role_policy(
        ...
        print(f"IAM ロール {lambda_role_name} は既に存在します")
...
def create_lambda_layer(packages, layer_name):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Install packages
        subprocess.run(
            ["pip", "install", "--target", python_dir] + packages, check=True
        )
        ...
        print(f"Lambda レイヤーを作成しました: {layer_arn}")
        return layer_arn
...
def create_lambda(
    name, code, lambda_role_arn, description, TAVILY_API_KEY=None, packages=None
):
    """
    Lambda 関数を作成する汎用関数

    コードを ZIP パッケージ化してデプロイし、必要に応じて依存パッケージ用の
    Lambda レイヤーも作成する。
    """
    layer_arn = None
    if packages:
