#!/usr/bin/env python3
"""
Bedrock-agentcore Browser ライブビューア（適切なディスプレイサイズサポートと DCV デバッグ機能付き）
"""
...
class BrowserViewerServer:
    """設定可能なディスプレイサイズを持つ Bedrock-agentcore Browser セッションを表示するためのサーバー"""
    ...
    def _create_static_files(self):
        """SDK に含まれる JavaScript と CSS ファイルを作成する"""
...
#debug-info {
    ...
    def _check_dcv_sdk(self):
        ...
        if not dcv_js_path.exists():
            ...
            console.print(f"Amazon DCV Web Client SDK が必要ですが、見つかりません。")
            ...
            if file_size < 10000:  # Real DCV SDK is much larger
                ...
                console.print(f"ファイルサイズ: {file_size} バイト (100KB 以上が期待されます)")
                console.print("実際の DCV SDK ファイルに置き換えてください\n")
            else:
                console.print(f"[green]✅ DCV SDK found ({file_size:,} bytes)[/green]")

    def _setup_routes(self):
        """FastAPI ルートをセットアップする"""
        ...
        print(f"ブラウザを使用したセッション: {self.browser_client.identifier}")
        print(f"セッション ID: {self.browser_client.session_id}")

    def _check_dcv_files(self):
        """どの DCV ファイルが存在するかを確認する"""
        dcv_files = {}
        dcv_dir = self.static_dir / "dcvjs"
