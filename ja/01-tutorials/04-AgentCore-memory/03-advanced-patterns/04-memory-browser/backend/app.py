#!/usr/bin/env python3
"""
AgentCore Memory ダッシュボードバックエンド - メモリレコード一覧表示専用
セマンティック検索なしですべてのメモリレコードを単純に一覧表示するアプローチ
"""
...
def get_aws_region():
    """環境変数、AWS CLI 設定、またはデフォルトから AWS リージョンを取得する"""
    ...
    try:
        ...
        if aws_path:
            ...
            logger.debug("AWS CLI が PATH に見つかりません")
    ...
    logger.warning("AWS リージョンが設定されていません。デフォルト: us-east-1 を使用します")
    return "us-east-1"
...
async def get_short_term_memory(query: ShortTermMemoryQuery):
    """AgentCore Memory から短期メモリ（イベントと会話ターン）を取得する"""
    try:
        ...
        logger.error(f"短期メモリの取得中にエラーが発生しました: {e}")
        clean_error = clean_aws_error_message(str(e))
        raise HTTPException(
...
async def search_event_by_id(query: EventSearchQuery):
    ...
    try:
        ...
        for actor_id in actor_ids_to_try:
            for session_id in session_ids_to_try:
                try:
                    ...
                    logger.warning(f"{actor_id}/{session_id} の検索に失敗しました: {e}")
                    continue
        ...
        logger.error(f"イベントの検索中にエラーが発生しました: {e}")
        clean_error = clean_aws_error_message(str(e))
        raise HTTPException(
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"代替イベント検索に失敗しました: {e}")
                clean_error = clean_aws_error_message(str(e))
                raise HTTPException(
        ...
        logger.error(f"ID でイベントを取得中にエラーが発生しました: {e}")
        clean_error = clean_aws_error_message(str(e))
        raise HTTPException(
...
async def list_namespaces_old(query: MemoryQuery):
    """AgentCore Memory 戦略から利用可能な namespace を一覧表示する（非推奨）"""
    try:
        if not memory_client:
            raise HTTPException(
                status_code=503, detail="AgentCore Memory client not available"
        ...
        try:
            ...
            error_msg = str(e).lower()
            logger.error(f"メモリストラテジーの取得に失敗しました: {e}")
        ...
        logger.error(f"名前空間の一覧取得中にエラーが発生しました: {e}")
        clean_error = clean_aws_error_message(str(e))
        raise HTTPException(
...
async def get_long_term_memory(query: LongTermMemoryQuery):
    """AgentCore Memory から長期メモリ（ファクト、プリファレンス、サマリー）を取得する"""
    try:
        if not memory_client:
            raise HTTPException(
                status_code=503, detail="AgentCore Memory client not available"
        ...
        logger.error(f"長期メモリの取得中にエラーが発生しました: {e}")
        clean_error = clean_aws_error_message(str(e))
        raise HTTPException(
...
async def get_memory_entries(query: MemoryQuery):
    """AgentCore Memory からすべてのメモリレコードを一覧表示する"""
    try:
        if not memory_client:
            raise HTTPException(
                status_code=503, detail="AgentCore Memory client not available"
        ...
        logger.error(f"メモリレコードの一覧取得中にエラーが発生しました: {e}")
        clean_error = clean_aws_error_message(str(e))
        raise HTTPException(
...
async def list_namespaces_v2(query: ListNamespacesQuery):
    """指定されたメモリ ID の利用可能な namespace を一覧表示する（v2）"""
    try:
        if not memory_client:
            raise HTTPException(
                status_code=503, detail="AgentCore Memory client not available"
        ...
        try:
            ...
            logger.warning(f"メモリストラテジーの取得に失敗しました: {e}")
            ...
            try:
                ...
                logger.warning(f"フォールバック方法も失敗しました: {e2}")
                return {
                    "namespaces": [],
                    "total_found": 0,
...
async def validate_memory_id(query: MemoryIdValidationQuery):
    ...
    try:
        ...
        logger.error(f"メモリ ID の検証中にエラーが発生しました: {e}")
        clean_error = clean_aws_error_message(str(e))
        raise HTTPException(
...
async def add_memory_entry(query: AddMemoryEntryQuery):
    """メモリエントリを追加する（AgentCore Memory では未実装）"""
    return {
        "success": False,
...
async def delete_memory_entries(query: DeleteMemoryEntriesQuery):
    """メモリエントリを削除する（AgentCore Memory では未実装）"""
    return {
        "success": False,
...
async def search_memory_entries(query: SearchMemoryEntriesQuery):
    ...
    try:
        ...
        logger.error(f"メモリエントリの検索中にエラーが発生しました: {e}")
        clean_error = clean_aws_error_message(str(e))
        raise HTTPException(
...
async def list_sessions():
    ...
    try:
        ...
        logger.error(f"セッションの一覧取得中にエラーが発生しました: {e}")
        return {
            "sessions": [],
            "total_sessions": 0,
