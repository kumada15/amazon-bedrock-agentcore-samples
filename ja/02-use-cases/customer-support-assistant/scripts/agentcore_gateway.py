...
def create_gateway(gateway_name: str, api_spec: List) -> dict:
    """指定された設定で AgentCore ゲートウェイを作成する。"""
    try:
...
@click.pass_context
def cli(ctx):
    """AgentCore ゲートウェイ管理 CLI。

    カスタマーサポートアプリケーション用の AgentCore ゲートウェイを作成・削除します。
    """
...
def delete(gateway_id, confirm):
    """AgentCore ゲートウェイとそのすべてのターゲットを削除する。"""
