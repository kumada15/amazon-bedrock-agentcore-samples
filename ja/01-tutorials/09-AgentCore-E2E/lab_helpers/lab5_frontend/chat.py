...
def invoke_endpoint_streaming(
    ...
    """エージェントエンドポイントを呼び出し、ストリーミングレスポンスチャンクを yield する。"""
    ...
    try:
        ...
        print("payload を JSON として解析できなかったため、payload オブジェクトにラップします")
    ...
    try:
        ...
        print("Agent エンドポイントの呼び出しに失敗しました: %s", str(e))
        raise


class ChatManager:
    def format_response_text(self, text):
        """クォートと改行をアンエスケープしてレスポンステキストをフォーマット"""
        if not text:
            return text
    ...
    def invoke_endpoint_nostreaming(
        ...
        """Bearer トークンを使用した HTTP リクエストでエージェントエンドポイントを呼び出す。"""
        escaped_arn = urllib.parse.quote(agent_arn, safe="")
        url = f"https://bedrock-agentcore.{st.session_state['region']}.amazonaws.com/runtimes/{escaped_arn}/invocations"
